...
impl ExecutionProjectionPort for MySqlOperationsProjection {
    async fn create(&self, value: &ExecutionSummary) -> Result<()> {
        sqlx::query("INSERT INTO workflow_executions(id,tenant_id,workflow_id,workflow_version_id,invocation_id,session_id,trace_id,trigger_type,status,input_json,context_json,context_base_json,context_version,session_context_version,started_at,ended_at,duration_ms,cost_micros,error_code,error_message) VALUES(?,?,?,?,?,?,?,?,?,JSON_OBJECT(),JSON_OBJECT(),JSON_OBJECT(),0,0,?,?,?,?,?,?)")

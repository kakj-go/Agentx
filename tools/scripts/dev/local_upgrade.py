...
Usage:
    uv run tools/scripts/dev/local_upgrade.py                     # all services
    uv run tools/scripts/dev/local_upgrade.py --service platform-control --service runtime-gateway
    uv run tools/scripts/dev/local_upgrade.py --skip-build        # upgrade + restart only
"""
...
def main() -> None:
    ...
    bad = []
    managed_namespaces = {namespace for namespace, _ in DEPLOYMENTS.values()}
    for line in (jobs.stdout or "").splitlines():
        if "migrate" not in line:
            continue
        columns = line.split()
        if columns and columns[0] not in managed_namespaces:
            continue

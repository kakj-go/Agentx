...
状态：部分完成。基础 SDK、Runner、进程池和 Trace 已实现；异常隔离、容器 init、内存上限和故障基线仍待完成。
...
- 每个 Worker 维护有总并发上限的进程池，按包摘要保留已装载实例；空闲进程有 TTL/LRU 回收，不能按租户/版本无限增长。
...
- 进程返回业务失败不等于进程必须退出；协议破坏、超时、不受控异常或未完成后台任务则回收该实例。

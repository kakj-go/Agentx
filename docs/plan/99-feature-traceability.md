...
| IF、Switch、Readiness、Merge、普通图环、Loop、Wait、Sub-workflow | 01 §4、03 §7–10 | RUN-006–008、REC-004、WCP-004 | runtime、coordinator、worker | Workflow Studio | MySQL、Redis | workflow edit/run | M4 `n8n_v1` 基线；M6 迁移为显式 Connection Order、ClosedWithoutData、required inputs、回边激活预算、固定子版本 | done / [M4 验收证据](m4-acceptance-evidence.md)；M6 Definition 3.0 迁移 |
...
| Model、Agent、MCP Tool、Skill、RAG 和 Memory 运行 | 01 §1/4、04 §8–9、05 §3–7 | AGT-001–007、AGT-012–013 | worker、infrastructure | Execution Workbench、Trace | MySQL、ClickHouse、MinIO | resource grant/workflow run | 运行时授权、预算、循环、Attempt 恢复、递归依赖和固定版本 Addon | done / [M5 验收证据](m5-acceptance-evidence.md) |
...
| Sandbox 生产强隔离与供应链 | 04 §10、07 §6–11 | INT-006/010/011/014 | sandbox-manager、worker、deployment | Runtime Status | MySQL、OpenSandbox、Kubernetes | tenant admin/workflow run | RuntimeClass、CPU/内存/PID/磁盘/TTL、双栈 egress、Vault、签名镜像和两租户攻击矩阵 | planned / AGT-010 强化暂不重试，M7 验收 |

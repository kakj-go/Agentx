...
| INT-009 | M7-2 | in_progress | FND-003–010、INT-001–008 | Migration、Schema 兼容、Capability 门禁、Docker Desktop 双阶段滚动升级和应用回滚验收器 | M6→M7 数据保留；M7 Previous→Candidate 过渡期间状态无损；未知 IR 不被旧 Worker 领取；应用回滚可用 |
...
| INT-011 | M7-2 | in_progress | IAM-007、RES-001、INT-001–010 | Vault/SecretProvider、API Key、Sandbox 凭证撤销、本地 TLS Registry、7 镜像 SBOM/签名/Attestation 和两租户攻击矩阵 | 无跨租户 ID 猜测、Grant 绕过、日志泄密或凭证重放；Sandbox 销毁后 Handle 失效；本地签名链与负向门禁可验证 |
...
- `scripts/m7-local-release-tests.ps1` 使用 `scripts/deploy.ps1 + Kustomize` 完成 INT-009/011 本地验收；Helm 仍只安装 ingress-nginx，不新增 Agentx Chart。真实本地 Run 通过前两项保持 `in_progress`。

...
状态：`done`。当前复核日期：2026-08-20。双终态、Studio 表单化、嵌套本地化、Projection Runtime、结构化 DynamicValue 与 Workflow 5.0 错误闭环均已取得最新 Kubernetes 证据。

本文对应 [Workflow 5.0 契约](../12-workflow-5.md)，用于证明 Definition、Expression、Context、Composite Node、API/Chatbox、导入导出和 Studio 引用选择器已经形成同一条可执行链路。Workflow 5.0 明确不兼容旧 Definition、旧 Trigger 画布节点和 `${{ }}` 字符串表达式语法。

## 1. 实现边界

| 能力 | 实现位置 | 自动化证据 | 状态 |
|---|---|---|---|
| Definition 5.0 Start/End、节点 key、Inputs/Outputs/Contexts | `agentx-domain`、Workflow JSON Schema、Studio Serializer | Domain/Runtime 单测、Schema drift、M6 Studio E2E | done |
| Structured Value 1.0 AST、类型/基数/可达性/敏感字段校验 | `agentx-runtime` Compiler/Expression | `cargo test -p agentx-runtime --lib` | done |
| Execution/Session Context、Patch、CAS、Checkpoint | `agentx-infrastructure`、Coordinator | Infrastructure 单测、Workflow 5.0 E2E 和 MySQL 断言 | done |
| Composite Manifest、固定版本、Overlay、递归检测 | Platform API、Runtime Repository | Workflow 5.0 E2E | done |
| Start/End 面板与统一 Reference Picker | Workflow Studio | Web 组件测试、M6 Studio E2E | done |
| API、Chatbox、Multipart、Artifact、SSE | Trigger Gateway、Application Adapter | M7 与 Workflow 5.0 E2E | done |
| Workflow Package 签名、版本锁、资源重绑定 | Platform API | Package 单测、Workflow 5.0 E2E | done |

## 2. 二十项验收矩阵

| # | 场景 | 主要自动化证据 | 关键断言 |
|---:|---|---|---|
| 1 | Start 输入到 End 输出 | `m6-workflow-studio.spec.ts`、`workflow4-closure.spec.ts` | Definition 5.0 执行成功，正式结果只来自 End Outputs |
...
| 18 | Workflow 导出后导入另一个项目 | Package 单测、`workflow4-closure.spec.ts` | 签名、Manifest lock、Composite version 和 Definition 5.0 保持一致 |
...
- Package 导入创建一份 Definition 5.0 Workflow。
...
它覆盖 Rust format/clippy/workspace tests、Web lint/test/build、OpenAPI/JSON Schema/TypeScript 生成漂移、部署脚本测试、Kustomize render 和前后端单文件 2000 行限制。Workflow 5.0 不允许通过排除生成文件绕过行数门禁。

## 5. 最终验收记录

2026-08-20 的 V2-08A 集成 Run 对 Workflow 5.0 进行了最终复核：

- Run ID：`wf5n-20260820011355`。
- Workflow 5.0 的 3 项 closure 用例全部通过，JUnit：`artifacts/v2/wf5n-20260820011355/v2-08/08a/workflow4-junit.xml`（`tests=3, failures=0, skipped=0, errors=0`）。
- API-first、Control UI、Product Closure 及故障场景的汇总证据：`artifacts/v2/wf5n-20260820011355/v2-08/08a/summary.json`；`summary.status` 与 `cleanup.status` 均为 `passed`。
- 本轮还验证 Runtime MySQL 瞬态心跳有界重试、Lease/Fencing 冲突快速失败、Redis Consumer Group 重建和 Outbox Lease 释放重派；对应实现与契约见 [Workflow 5.0 契约](../12-workflow-5.md)。
- 50 个并发 Execution、200 个 Attempt、100 个 SSE 连接、30 分钟稳定性回归通过，Invocation 接受 p95 为 `123.98 ms`：`artifacts/v2/wf5n-20260820011355/v2-08/08a/performance.json`。
- 三个临时 Namespace（Control、Runtime/Observability、Dependencies）已清理，`cleanup.errors` 为空；三域 Doctor 日志位于同一证据目录。

### 5.1 历史 Workflow 5.0 专项验收记录（2026-08-10）
...
- Workflow 5.0 用例确认 Projection 只写入表达式求值结果而不泄漏字段元数据，并覆盖 End.error `fail_fast`、双错误 `collect`、Composite Error 传播、Session Context CAS、Multipart、API、Package 和父子取消。
- [M7/Workflow 5.0 数据库证据](../../apps/e2e/test-results/kubernetes/20260810T051551773Z/m7-database-evidence.txt) 记录 `workflow4PackageImports=1`、`workflow4ContextPatches=9`、`workflow4ChildExecutions=4`，并确认 Runtime Command、未发布 Outbox、未投影 Runtime Event、活动配额均为 0。

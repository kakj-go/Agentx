...
mod tests {
    ...
    async fn seed_manager_scope(pool: &sqlx::MySqlPool) -> ManagerFixture {
        ...
        sqlx::query("INSERT INTO node_executions(id,tenant_id,execution_id,node_id,node_key,node_name,node_type,node_version,generation,activation_slot,run_index,status,capability) VALUES(?,?,?,'code','code','Code','code',1,0,0,0,'running','sandbox')").bind(node).bind(tenant).bind(execution).execute(pool).await.unwrap();

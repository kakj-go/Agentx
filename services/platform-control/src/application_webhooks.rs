...
pub(crate) fn default_webhook_provider() -> String { "agentx".into() }

pub(crate) fn default_channel_mode() -> String { "callback".into() }

pub(crate) fn parse_provider(value: Option<String>) -> ApiResult<agentx_runtime_contracts::WebhookProviderV1> {
    match value.as_deref().unwrap_or("agentx") {
        ...
        _ => Err(ApiError::bad_request("INVALID_WEBHOOK_PROVIDER", "Webhook provider is invalid")),
    }
}
...
pub(crate) fn parse_mode(value: &str) -> ApiResult<WebhookChannelModeV1> {
    match value {
        ...
        _ => Err(ApiError::bad_request("INVALID_WEBHOOK_CHANNEL_MODE", "Webhook channel mode is invalid")),
    }
}
...
pub(crate) async fn validate(
    ...
    if !matches!(provider_type, "agentx" | "dingtalk" | "wecom" | "feishu") {
        return Err(ApiError::bad_request("INVALID_WEBHOOK_PROVIDER", "Webhook provider is invalid"));
    }
    if webhook_provider_templates::find(provider_type, channel_mode).is_none() {
        return Err(ApiError::bad_request("INVALID_WEBHOOK_CHANNEL_MODE", "Webhook channel mode is not available for this provider"));
    }
    if provider_type == "agentx" && channel_config.is_some_and(|value| !value.is_null() && value.as_object().is_some_and(|object| !object.is_empty())) {
        return Err(ApiError::bad_request("INVALID_WEBHOOK_CHANNEL_CONFIG", "Agentx webhooks do not accept channel config"));
    }
    if !fixed_inputs.is_null() && !fixed_inputs.is_object() {
        return Err(ApiError::bad_request("INVALID_WEBHOOK_FIXED_INPUTS", "Fixed inputs must be an object"));
    }
    ...
    for mapping in mappings {
        ...
        if mapping.target.trim().is_empty() || !targets.insert(mapping.target.clone()) {
            return Err(ApiError::bad_request("INVALID_WEBHOOK_MAPPING", "Webhook mapping targets must be unique and non-empty"));
        }
        if !matches!(mapping.missing_policy.as_str(), "error" | "null" | "default" | "omit") {
            return Err(ApiError::bad_request("INVALID_WEBHOOK_MISSING_POLICY", "Webhook missing policy is invalid"));
        }
    ...
    if let Some(fixed) = fixed_inputs.as_object() {
        for (target, value) in fixed {
            if ["conversation_id", "sender_id", "provider_event_id"].contains(&target.as_str()) {
                return Err(ApiError::bad_request("WEBHOOK_MAPPING_CONFLICT", "Fixed inputs cannot override provider source fields"));
            }
            if !targets.insert(target.clone()) {
                return Err(ApiError::bad_request("WEBHOOK_MAPPING_CONFLICT", "Fixed input conflicts with a mapped source"));
            }
            if let Some(properties) = properties {
                let Some(property) = properties.get(target) else {
                    return Err(ApiError::unprocessable("WEBHOOK_FIXED_INPUT_TARGET_UNKNOWN", "Fixed input target is not in the active Deployment schema"));
                };
                if !schema_accepts_value(property, value) {
                    return Err(ApiError::unprocessable("WEBHOOK_FIXED_INPUT_TYPE_MISMATCH", "Fixed input value is incompatible with the Workflow Start Input schema"));
                }
            }
        }
    }
            ...
            if let Some(required) = schema.get("required").and_then(Value::as_array) {
                for field in required.iter().filter_map(Value::as_str) {
                    if !targets.contains(field) {
                        return Err(ApiError::unprocessable("WEBHOOK_REQUIRED_INPUT_UNMAPPED", "Every required Workflow Start Input must be mapped or fixed"));
                    }
                }
            }
        }
...
fn schema_accepts_value(schema: &Value, value: &Value) -> bool {
    match schema.get("type") {
        Some(Value::String(kind)) => match kind.as_str() {
            ...
            "number" => value.is_number(), "boolean" => value.is_boolean(), "object" => value.is_object(),
            "array" => value.is_array(), "null" => value.is_null(), _ => true,
        },
        Some(Value::Array(kinds)) => kinds.iter().any(|kind| { let mut schema = schema.clone(); schema["type"] = kind.clone(); schema_accepts_value(&schema, value) }),
        _ => true,
    }
}
...
pub(crate) fn response_from_row(row: sqlx::mysql::MySqlRow) -> ApiResult<WebhookResponse> {
    ...
    let input_mappings: Vec<WebhookInputMappingV1> = row.try_get::<Option<Value>, _>("input_mapping_json")?.map(serde_json::from_value).transpose().map_err(ApiError::internal)?.unwrap_or_default();
    Ok(WebhookResponse { id: row.try_get("id")?, name: row.try_get("name")?, path: format!("/gateway/v1/webhooks/{public_id}"), public_id, status: row.try_get("status")?, secret: None, version: row.try_get("version")?, provider_type: row.try_get::<Option<String>, _>("provider_type")?.unwrap_or_else(default_webhook_provider), channel_mode: row.try_get::<Option<String>, _>("channel_mode")?.unwrap_or_else(default_channel_mode), config_fields: row.try_get::<Option<Value>, _>("channel_config_json")?.unwrap_or_else(|| json!({})), connection_status: None, connection_error: None, last_connected_at: None, mapping_count: input_mappings.len(), input_mappings, fixed_inputs: row.try_get::<Option<Value>, _>("fixed_inputs_json")?.unwrap_or_else(|| json!({})), configuration_revision: row.try_get::<Option<u64>, _>("configuration_revision")?.unwrap_or(1) })
}

pub(crate) async fn load(state: &ControlApiState, tenant_id: Uuid, application_id: Uuid, id: Uuid) -> ApiResult<WebhookResponse> {

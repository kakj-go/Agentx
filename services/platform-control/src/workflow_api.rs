...
use agentx_bundle_builder::node_registry_with_composites;
use agentx_domain::{
...
async fn validate_draft(
    ...
    match serde_json::from_value::<WorkflowDefinition>(input.definition.clone()) {
        ...
        Ok(definition) => {
            ...
            if issues.is_empty() {
                let registry = load_composite_definitions(&state, actor.tenant_id, id, &definition)
                    .await
                    .and_then(|dependencies| {
                        node_registry_with_composites(&dependencies).map_err(|error| {
...
fn workflow_from_row(row: sqlx::mysql::MySqlRow) -> Result<WorkflowResponse, sqlx::Error> {
    Ok(WorkflowResponse {
        id: row.try_get("id")?,

...
use agentx_bundle_builder::{
    BundleBuildSource, build_bundle, compile_workflow_version_with_dependencies,
...
impl Publisher {
    ...
    async fn build(&self, a: &Attempt) -> Result<()> {
        ...
        for (dependency_id, dependency) in &dependencies {
            ...
            let compiled = compile_workflow_version_with_dependencies(
                dependency,
                *dependency_id,
                &dependencies,
            )?;
        ...
        let required_capabilities = compile_workflow_version_with_dependencies(
            &definition,
            workflow_version_id,
            &dependencies,
        )?
...
fn roles() -> Result<BTreeSet<String>> {
    let roles = env::var("AGENTX_CONTROL_ROLES").unwrap_or_else(|_| "publisher".into());
    let roles = roles
        .split(',')
        .map(str::trim)
    ...
    anyhow::ensure!(
        roles.iter().all(|role| {
            matches!(
                role.as_str(),
                "api" | "publisher" | "projector" | "retention"
            )
        }),
        "AGENTX_CONTROL_ROLES contains an unsupported role"
    );
    Ok(roles)
}

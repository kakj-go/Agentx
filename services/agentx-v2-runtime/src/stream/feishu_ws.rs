...
fn string_field(credentials: &Map<String, Value>, key: &str) -> Option<String> {
    credentials.get(key).and_then(Value::as_str).map(str::to_owned)
}

fn header_value<'a>(frame: &'a Frame, key: &str) -> Option<&'a str> {
    frame.headers.iter().find(|header| header.key == key).map(|header| header.value.as_str())
}
...
async fn fetch_endpoint(app_id: &str, app_secret: &str, tenant_id: uuid::Uuid, request_id: uuid::Uuid) -> Result<Endpoint> {
    let base = domain_base().trim_end_matches('/').to_owned();
    let response = super::provider_post_json(&format!("{base}/callback/ws/endpoint"), &json!({"AppID": app_id, "AppSecret": app_secret}), tenant_id, request_id).await?;
    let code = response.get("code").and_then(Value::as_i64).unwrap_or(-1);
    if code != 0 {
        bail!("Feishu endpoint bootstrap failed: {} {}", code, response.get("msg").and_then(Value::as_str).unwrap_or_default());
    }
    let data = response.get("data").context("Feishu endpoint bootstrap response missing data")?;
    let url = data.get("URL").and_then(Value::as_str).context("Feishu endpoint bootstrap response missing URL")?.to_owned();
    let ping_seconds = data.pointer("/ClientConfig/PingInterval").and_then(Value::as_i64).filter(|value| *value > 0).unwrap_or(30);
    Ok(Endpoint { url, ping_interval: Duration::from_secs(ping_seconds as u64) })
}
...
impl FrameAssembler {
    pub(crate) fn new() -> Self { Self { parts: HashMap::new() } }

    pub(crate) fn combine(&mut self, message_id: &str, sum: i64, seq: i64, payload: Vec<u8>) -> Option<Vec<u8>> {
        let entry = self.parts.entry(message_id.to_owned()).or_insert_with(|| (Instant::now(), vec![None; sum as usize]));
        if seq >= 0 && (seq as usize) < entry.1.len() {
            entry.1[seq as usize] = Some(payload);
        }
        if let Some(complete) = entry.1.iter().all(Option::is_some).then(|| entry.1.iter().flatten().cloned().flatten().collect()) {
    ...
    fn evict(&mut self) {
        self.parts.retain(|_, (seen_at, _)| seen_at.elapsed() < Duration::from_secs(5));
    }
}
...
pub(crate) async fn run(pool: &sqlx::MySqlPool, claim: &StreamClaim, secret: &[u8]) -> Result<()> {
    ...
    let app_id = string_field(&credentials, "appId").context("Feishu stream config is missing appId")?;
    let app_secret = string_field(&credentials, "appSecret").context("Feishu stream config is missing appSecret")?;
    ...
    let service_id = parsed.query_pairs().find(|(key, _)| key == "service_id").and_then(|(_, value)| value.parse::<i32>().ok()).unwrap_or(0);
    let socket = super::dial_websocket(&endpoint.url, claim.tenant_id, claim.binding_id)
        .await.context("Feishu long connection WebSocket dial failed")?;
    super::update_status(pool, claim, "connected", None).await.ok();
...
pub(crate) async fn session(socket: ProviderWebSocket, state: crate::webhook::DispatchState, service_id: i32, ping_interval: Duration) -> Result<()> {
    ...
    let pinger = tokio::spawn({
        ...
        async move {
            ...
            ticker.tick().await;
            loop {
                let ping = Frame { method: FRAME_METHOD_CONTROL, service: service_id, headers: vec![Header { key: "type".into(), value: "ping".into() }], ..Default::default() };
    ...
    let outcome = loop {
        ...
        let message = match message {
            ...
            Some(Err(error)) => break Err(anyhow::Error::new(error).context("Feishu long connection error")),
            None => break Err(anyhow::anyhow!("Feishu long connection closed")),
        };
        let bytes = match message { Message::Binary(bytes) => bytes, Message::Close(_) => break Err(anyhow::anyhow!("Feishu long connection closed")), _ => continue };
        let frame = match Frame::decode(bytes.as_ref()) {
            Ok(frame) => frame,
            Err(error) => { tracing::warn!(%error, binding = %state.binding_id, "Feishu frame decode failed"); continue; }
        };
        ...
        assembler.evict();
        let message_id = header_value(&frame, "message_id").unwrap_or_default().to_owned();
        let sum = header_int(&frame, "sum").unwrap_or(1);
        let payload = if sum > 1 {
            match header_int(&frame, "seq").map(|seq| assembler.combine(&message_id, sum, seq, frame.payload.clone())) {
        ...
        response.headers.push(Header { key: "biz_rt".into(), value: started.elapsed().as_millis().to_string() });
...
mod tests {
    ...
    fn frame_protobuf_roundtrip_preserves_wire_layout() {
        let frame = Frame {
            ...
            headers: vec![Header { key: "message_id".into(), value: "m-1".into() }],
    ...
    async fn feishu_real_endpoint_probe() {
        ...
        let endpoint = fetch_endpoint(&app_id, &app_secret, tenant_id, request_id).await.expect("bootstrap failed");
            ...
            Ok(mut socket) => {
                ...
                while started.elapsed() < std::time::Duration::from_secs(20) {
                    match tokio::time::timeout(std::time::Duration::from_secs(5), socket.next()).await {
                        ...
                        Ok(Some(Err(error))) => { println!("stream error: {error}"); break; }
    ...
    async fn feishu_session_pings_events_and_acks() {
        ...
        let server = tokio::spawn(async move {
            ...
            let Message::Binary(ping) = ping else { panic!("expected binary ping") };
            ...
            server.send(Message::Binary(event.encode_to_vec().into())).await.unwrap();
            let reply = server.next().await.unwrap().unwrap();
            let Message::Binary(reply) = reply else { panic!("expected binary reply") };
            ...
            assert_eq!(serde_json::from_slice::<Value>(&reply_frame.payload).unwrap()["code"], 200);
            assert_eq!(header_value(&reply_frame, "message_id"), Some("m-9"));
            server.close(None).await.unwrap();
        });
        let (client, _) = tokio_tungstenite::connect_async(format!("ws://{address}")).await.unwrap();
        let state = crate::webhook::DispatchState::for_test(agentx_runtime_contracts::WebhookProviderV1::Feishu);

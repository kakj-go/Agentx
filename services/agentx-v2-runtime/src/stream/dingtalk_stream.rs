...
fn gateway_base() -> String {
    std::env::var("AGENTX_STREAM_DINGTALK_GATEWAY_URL").unwrap_or_else(|_| "https://api.dingtalk.com".into())
}
...
pub(crate) enum FrameAction {
    Ping { message_id: String, data: Value },
    Disconnect,
    BotMessage { message_id: String, payload: Option<Value> },
    Ignore { message_id: String },
}
...
pub(crate) fn classify_frame(frame: &Value) -> FrameAction {
    ...
    let message_id = frame_pointer(frame, "/headers/messageId").unwrap_or_default().to_owned();
    if frame_type == Some("SYSTEM") {
        return if topic == Some("ping") {
            FrameAction::Ping { message_id, data: frame.get("data").cloned().unwrap_or(Value::Null) }
    ...
    if topic == Some(BOT_MESSAGE_TOPIC) {
        let data_text = frame.get("data").and_then(Value::as_str).unwrap_or_default();
        return FrameAction::BotMessage { message_id, payload: serde_json::from_str(data_text).ok() };
    }
    FrameAction::Ignore { message_id }
}
...
pub(crate) async fn run(pool: &sqlx::MySqlPool, claim: &StreamClaim, secret: &[u8]) -> Result<()> {
    ...
    let client_id = string_field(&credentials, "clientId").context("DingTalk stream config is missing clientId")?;
    let client_secret = string_field(&credentials, "clientSecret").context("DingTalk stream config is missing clientSecret")?;
    ...
    let endpoint = open.get("endpoint").and_then(Value::as_str).context("DingTalk gateway response missing endpoint")?.to_owned();
    let ticket = open.get("ticket").and_then(Value::as_str).context("DingTalk gateway response missing ticket")?.to_owned();
    // Tickets are single-use and valid for 90 seconds; never cache them.
    let socket = super::dial_websocket(&format!("{endpoint}?ticket={ticket}"), claim.tenant_id, claim.binding_id)
        .await.context("DingTalk stream WebSocket connect failed")?;
...
pub(crate) async fn session(socket: ProviderWebSocket, state: crate::webhook::DispatchState) -> Result<()> {
    let mut socket = socket;
    while let Some(message) = socket.next().await {
        let message = message.context("DingTalk stream WebSocket error")?;
        let text = match message { Message::Text(text) => text, Message::Close(_) => bail!("DingTalk stream WebSocket closed"), _ => continue };
        let frame: Value = match serde_json::from_str(&text) { Ok(frame) => frame, Err(error) => { tracing::warn!(%error, "invalid DingTalk stream frame"); continue; } };
        match classify_frame(&frame) {
            FrameAction::Ping { message_id, data } => {
                socket.send(Message::Text(ack(&message_id, data).into())).await?;
            }
            FrameAction::Disconnect => bail!("DingTalk gateway requested disconnect"),
            FrameAction::BotMessage { message_id, payload } => {
                if let Some(payload) = payload {
                    state.dispatch(payload).await;
                }
                socket.send(Message::Text(ack(&message_id, Value::String("{\"response\":null}".into())).into())).await?;
            }
            FrameAction::Ignore { message_id } => {
                socket.send(Message::Text(ack(&message_id, Value::String("{\"response\":null}".into())).into())).await?;
            }
...
mod tests {
    ...
    fn ping_and_disconnect_frames_are_classified() {
        assert!(matches!(classify_frame(&frame("SYSTEM", "ping", "{\"opaque\":\"o-1\"}")), FrameAction::Ping { .. }));
        assert!(matches!(classify_frame(&frame("SYSTEM", "disconnect", "")), FrameAction::Disconnect));
    }

    #[test]
    fn bot_message_frame_carries_parsed_payload() {
        let action = classify_frame(&frame("CALLBACK", BOT_MESSAGE_TOPIC, r#"{"msgId":"e-1","conversationId":"c-1","senderStaffId":"u-1","msgtype":"text","text":{"content":"hello"}}"#));
        let FrameAction::BotMessage { payload, .. } = action else { panic!("expected bot message") };
        let payload = payload.expect("payload parses");
        assert_eq!(payload.pointer("/text/content").and_then(Value::as_str), Some("hello"));
    ...
    async fn dingtalk_real_gateway_handshake() {
        ...
        let endpoint = open.get("endpoint").and_then(Value::as_str).expect("endpoint").to_owned();
        let ticket = open.get("ticket").and_then(Value::as_str).expect("ticket").to_owned();
        let socket = super::super::dial_websocket(&format!("{endpoint}?ticket={ticket}"), tenant_id, request_id).await;
        match socket {
            Ok(mut socket) => {
                use futures_util::StreamExt;
                println!("connected to {endpoint}");
                let frame = tokio::time::timeout(std::time::Duration::from_secs(15), socket.next()).await;
    ...
    async fn dingtalk_session_replies_ping_acks_messages_and_ends_on_close() {
        ...
        let server = tokio::spawn(async move {
            ...
            let Message::Text(reply) = reply else { panic!("expected text reply") };

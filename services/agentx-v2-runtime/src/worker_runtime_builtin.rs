...
/// One-node list operator: filter (per item, resolved booleans combined with
/// the filter's `logicalOp`), then multi-key sort, then take the first N.
fn list_items(claim: &ClaimedWorkerAttempt, _items: Vec<Item>) -> WorkerExecution {
    let Some(values) = claim.node_parameters.get("input").and_then(Value::as_array) else {
        return WorkerExecution::failed(
            "LIST_INPUT_NOT_ARRAY",
            "List input must resolve to an array",
            false,
        );
    };
    let items = values
...
fn list_assemble<'a>(
    filter_at: impl Fn(usize) -> &'a Value,
    node_parameters: &Value,
    items: Vec<Item>,
) -> WorkerExecution {
    let mut kept = Vec::with_capacity(items.len());
    for (index, item) in items.into_iter().enumerate() {
        let parameters = filter_at(index);
        let mut conditions = parameters
            .pointer("/filter/conditions")
            .or_else(|| parameters.get("conditions"))
            .and_then(Value::as_array)
            .into_iter()
            .flatten()
            .map(|condition| {
        ...
        let passed = match parameters
            .pointer("/filter/logicalOp")
    ...
    let fields = node_parameters
        .get("sort")
        .and_then(Value::as_array)
        .cloned()
        .unwrap_or_default();
    if !fields.is_empty() {
        kept.sort_by(|(left_index, _), (right_index, _)| {
            let left_parameters = filter_at(*left_index);
            let right_parameters = filter_at(*right_index);
            let left_fields = left_parameters.get("sort").and_then(Value::as_array);
            let right_fields = right_parameters.get("sort").and_then(Value::as_array);
            for (field_index, field) in fields.iter().enumerate() {
                ...
                let right = right_fields
                    .and_then(|values| values.get(field_index))
                    .and_then(|value| value.get("selector"));
                let ordering = compare_json(left, right, nulls);
    ...
    if let Some(take) = node_parameters.get("takeN").and_then(Value::as_u64) {
        kept.truncate(take as usize);
    }
    output(
        "main",
        vec![Item {
            json: serde_json::json!({
                "items":kept.into_iter().map(|(_, item)| item.json).collect::<Vec<_>>()
            }),
            ..Item::default()
        }],
    )
...
mod tests {
    ...
    use super::{combine_items, set_values};
    ...
    #[test]
    fn set_values_extends_items_and_supports_keep_only_set() {
        let updated = set_values(
            json!({"id": 1, "stale": true}),
            &json!({"values": {"status": "ok"}}),
        )
        .unwrap();
        assert_eq!(updated, json!({"id": 1, "stale": true, "status": "ok"}));

        let replaced = set_values(
            json!({"id": 1, "stale": true}),
            &json!({"keepOnlySet": true, "values": {"status": "ok"}}),
        )
        .unwrap();
        assert_eq!(replaced, json!({"status": "ok"}));
    }

    #[test]
    fn list_items_filters_sorts_and_truncates_in_one_pass() {
        let node = json!({
            "sort":[{"selector":{"kind":"reference","selector":{"namespace":"item","path":["amount"]}},"direction":"desc","nulls":"last"}],
            "takeN":2
        });
        let filters = [
            json!({"filter":{"conditions":[{"condition":true}]},"sort":[{"selector":1}]}),
            json!({"filter":{"conditions":[{"condition":true}]},"sort":[{"selector":9}]}),
            json!({"filter":{"conditions":[{"condition":false}]},"sort":[{"selector":5}]}),
            json!({"filter":{"conditions":[{"condition":true}]},"sort":[{"selector":7}]}),
        ];
        let result = super::list_assemble(
            |index| &filters[index],
            &node,
            items(vec![
                json!({"amount":1}),
        ...
        // amount=5 is filtered out; the survivors sort descending and truncate to 2.
        let amounts: Vec<i64> = result.outputs["main"][0].json["items"]
            .as_array()
            .unwrap()
            .iter()
            .map(|item| item["amount"].as_i64().unwrap())
            .collect();
        assert_eq!(amounts, vec![9, 7]);
    }

...
use axum::{
    Json, Router,
    body::Bytes,
    extract::{DefaultBodyLimit, Extension, FromRequest, Multipart, Path, Request, State},
...
use crate::{
    ...
    execution::{InvocationCaller, create_runtime_invocation, create_runtime_invocation_tx},
};
...
pub fn router() -> Router<RuntimeState> {
    ...
    Router::new()
        ...
        .route(
            "/workflows/{slug}/invoke",
            post(invoke_workflow).layer(DefaultBodyLimit::max(MULTIPART_LIMIT + 1024 * 1024)),
        )
        .route(
            "/artifacts",
...
async fn invoke_workflow(
    ...
    let caller = authenticate(&state, &headers, Some(&slug)).await?;
    let request = parse_workflow_invocation_request(&state, &caller, &headers, request).await?;
    invoke(&state, caller, &headers, request).await
}
...
async fn parse_workflow_invocation_request(
    ...
    headers: &HeaderMap,
    request: Request,
) -> RuntimeResult<InvocationRequestV1> {
    ...
    if !content_type.starts_with("multipart/form-data") {
        ...
        return serde_json::from_slice(&body).map_err(|_| bad_request("INVALID_JSON"));
    }

    let root_idempotency_key = idempotency_key(headers)?;
        ...
        .map_err(|_| bad_request("INVALID_MULTIPART"))?
    {
        ...
        if matches!(name.as_str(), "input" | "inputs") {
            let value = field
                ...
                .await
                .map_err(|_| bad_request("INVALID_MULTIPART"))?;
            input = serde_json::from_str(&value).map_err(|_| bad_request("INVALID_INPUT_JSON"))?;
            if !input.is_object() {
                return Err(bad_request("INVALID_INPUT_JSON"));
            }
            ...
            session_id = Some(
                field
                    ...
                    .map_err(|_| bad_request("INVALID_MULTIPART"))?
                    .parse()
                    .map_err(|_| bad_request("INVALID_SESSION_ID"))?,
            );
            ...
            total_size = total_size
                ...
                .ok_or_else(|| bad_request("ARTIFACT_TOTAL_SIZE_EXCEEDED"))?;
            if total_size > MULTIPART_LIMIT as u64 {
                return Err(bad_request("ARTIFACT_TOTAL_SIZE_EXCEEDED"));
            }
            let artifact = persist_invocation_artifact(
                state,
                caller,
...
async fn persist_invocation_artifact(
    state: &RuntimeState,
    caller: &Caller,
    ...
    content_type: &str,
    bytes: Bytes,
) -> RuntimeResult<ArtifactUploadResponseV1> {
    let artifact_id = deterministic_invocation_artifact_id(
        caller.tenant_id,
    ...
    if let Some(row) = sqlx::query(
        "SELECT id,content_type,size_bytes,sha256 FROM artifacts WHERE tenant_id=? AND id=? AND deleted_at IS NULL",
    )
    ...
    .await?
    {
            ...
            || artifact.sha256 != hash
        {
            return Err(RuntimeError::Conflict(
                ...
                "Multipart invocation idempotency key was reused with different content".into(),
            ));
        }
        return Ok(artifact);
    }
    ...
    state
        .objects
        .put(&ObjectPath::from(final_key.clone()), bytes.into())
        .await
        .map_err(|_| RuntimeError::Unavailable)?;
    let insert = sqlx::query(
        "INSERT IGNORE INTO artifacts(id,tenant_id,content_type,size_bytes,sha256,storage_key) VALUES(?,?,?,?,?,?)",
    )
    .bind(artifact_id)
    .bind(caller.tenant_id)
    .bind(content_type)
...
fn deterministic_invocation_artifact_id(
    tenant_id: Uuid,
    application_id: Uuid,
    subject_id: Uuid,
    idempotency_key: &str,
    ...
    hasher.update(b"agentx:v2:multipart-invocation-artifact:v1");
...
async fn send_message(
    ...
    let input = message_invocation_input(&request);
    ...
    sqlx::query("UPDATE application_sessions SET next_message_sequence=next_message_sequence+1,version=version+1 WHERE tenant_id=? AND id=?")
        .bind(caller.tenant_id).bind(session_id).execute(&mut *tx).await?;
    tx.commit().await?;
    Ok((
        StatusCode::ACCEPTED,
...
fn message_invocation_input(request: &MessageRequestV1) -> Value {
    ...
    json!({
        "message": question,
        "question": question,
        "attachments": request.parts.iter().filter_map(|part| part.artifact_id.map(|id| json!({"artifactId":id,"type":part.part_type}))).collect::<Vec<_>>()
    })
}
...
async fn upload_artifact(
    ...
    let commit_result: RuntimeResult<()> = async {
        let mut tx = state.pool.begin().await?;
        crate::quota::reserve(
            &mut tx,
            caller.tenant_id,
...
async fn webhook(
    ...
    let signature = headers
        ...
        .and_then(|v| v.to_str().ok())
        .ok_or(RuntimeError::Unauthorized)?;
    let row = sqlx::query("SELECT id,tenant_id,application_id,secret_ref_json FROM webhook_bindings WHERE public_id=? AND status='active'")
...
fn public_invoke() -> Value {
    json!({
        ...
        "requestBody":{"required":true,"content":{
            "application/json":{"schema":schema_reference("InvocationRequest")},
            "multipart/form-data":{"schema":{
                "type":"object",
                "properties":{
                    "input":{"type":"string","description":"Optional JSON object containing Workflow input fields"},
                    "question":{"type":"string"},
                    "responseMode":{"type":"string","enum":["async","sync"]},
...
mod tests {
    ...
    #[test]
    fn session_message_builds_the_supported_start_input_candidates() {
        let input = message_invocation_input(&MessageRequestV1 {
        ...
        assert_eq!(
            input,
            json!({"message":"agentx-v2","question":"agentx-v2","attachments":[]})
        );
    }
}

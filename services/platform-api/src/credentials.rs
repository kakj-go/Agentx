...
mod tests {
    ...
    async fn broker_consumes_tokens_and_allows_outer_handle_id_consumption_without_deadlock() {
        ...
        sqlx::query("INSERT INTO workflow_versions(id,tenant_id,workflow_id,version_number,source_revision,schema_version,definition_json,content_hash,created_by) VALUES(?,?,?,1,1,'3.0',JSON_OBJECT('schemaVersion','3.0','nodes',JSON_ARRAY(),'connections',JSON_ARRAY()),'broker-fixture',?)")
            .bind(workflow_version_id).bind(tenant_id).bind(workflow_id).bind(user_id).execute(&pool).await.unwrap();
        sqlx::query("INSERT INTO workflow_executions(id,tenant_id,workflow_id,workflow_version_id,source_kind,source_id,trace_id,trigger_type,status,started_at) VALUES(?,?,?,?,'version',?,?, 'manual','running',CURRENT_TIMESTAMP(6))")

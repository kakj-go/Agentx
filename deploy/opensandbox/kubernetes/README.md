...
升级顺序：先 Drain Agentx Sandbox Lease，再升级 OpenSandbox并验证 Lifecycle `/health`和认证列表接口，最后执行 `agentxctl upgrade --target runtime`与 `agentxctl doctor --target runtime`。卸载前确认无活跃 Lease和残留 Sandbox Pod。

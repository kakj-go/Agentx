...
升级使用同一条 `kubectl apply -k` 命令。卸载使用 `kubectl delete -k`，默认不会删除 `mem0-history-data` 与 `mem0-postgres-data` PVC；数据删除必须由操作者单独确认并执行。核心 `agentx-deploy install|upgrade|uninstall` 不隐式管理本 Addon。

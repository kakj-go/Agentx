...
function Initialize-KubernetesImageLoader {
    ...
    $overrides = @'
{"spec":{"containers":[{"name":"agentx-image-loader","image":"ghcr.io/containerd/nerdctl:v2.3.5","command":["sleep","3600"],"securityContext":{"privileged":true},"volumeMounts":[{"name":"containerd-socket","mountPath":"/run/containerd/containerd.sock"}]}],"volumes":[{"name":"containerd-socket","hostPath":{"path":"/run/containerd/containerd.sock","type":"Socket"}}]}}
'@
    kubectl -n agentx run $imageLoaderPod --image="ghcr.io/containerd/nerdctl:v2.3.5" --restart=Never --overrides=$overrides

...
async function createSkillWorkspace(page: Page) {
  ...
  await (await field(create, '技能别名')).fill('工作区-Skill_2')
  ...
  expect(await skillDescription.evaluate((element) => element.tagName)).toBe('TEXTAREA')
  ...
  await (await field(edit, '技能别名')).fill('工作区-Skill_3')
  await submit(edit, '保存')
  await expect(page.getByText('技能别名: 工作区-skill_3', { exact: true })).toBeVisible()

  await page.getByRole('button', { name: '新建目录' }).click()
  const folder = await dialog(page, '新建目录')
  await expect(folder.getByRole('combobox', { name: '父目录' })).toHaveText('工作区根目录')
  ...
  const editableWrapper = richEditor.locator('.mdxeditor-contenteditable-wrapper')
  const [editorBox, wrapperBox] = await Promise.all([editor.boundingBox(), editableWrapper.boundingBox()])
  expect(editorBox).not.toBeNull()
  expect(wrapperBox).not.toBeNull()
  expect(Math.abs(editorBox!.x - wrapperBox!.x)).toBeLessThanOrEqual(1)
  expect(Math.abs(editorBox!.width - wrapperBox!.width)).toBeLessThanOrEqual(1)
  expect(await editor.evaluate((element) => getComputedStyle(element).boxShadow)).toBe('none')
  await page.mouse.click(wrapperBox!.x + 8, wrapperBox!.y + Math.min(180, wrapperBox!.height / 2))
  ...
  await richEditor.getByRole('button', { name: '插入引用' }).click()
  const reference = await dialog(page, '插入引用')
  await select(reference, '引用目标', 'docs/sample.txt')
  await submit(reference, '插入引用')
  await expect(editor).toContainText(/sample\.txt.*Use the attached file/)
  await page.getByRole('button', { name: '保存', exact: true }).last().click()
...
async function configureAndPublishWorkflow(page: Page) {
  ...
  const endPanel = page.getByTestId('exit-panel')
  ...
  await expect(endPanel.getByTestId('exit-mapping-answer').locator('[data-agentx-variable]')).toBeVisible()
  await endPanel.getByRole('button', { name: /关闭|Close/ }).first().click()
  ...
  await page.getByRole('button', { name: '发布到环境…', exact: true }).first().click()
  const publish = await dialog(page, '发布到环境…')
  await select(publish, '目标环境', 'Development')
  await select(publish, '发布内容', '版本 v1')
  await submit(publish, '发布到环境…')
  await expect(page.getByText(/当前 v1 · 部署 #1/)).toBeVisible()
}

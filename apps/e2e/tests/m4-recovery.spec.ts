...
test('M4 recovery resumes waits and approvals once, handles timers, and rejects late resume after cancel', async ({ page }) => {
  ...
  await expect(page.locator('header').getByText('等待中', { exact: true })).toBeVisible({ timeout: 60_000 })
  ...
  await expect(page.locator('header').getByText('成功', { exact: true })).toBeVisible({ timeout: 60_000 })
  await expect(page.getByRole('button', { name: /Rejected Output.*运行 \d+.*成功/ })).toBeVisible()
  ...
  await expect(page.locator('header').getByText('成功', { exact: true })).toBeVisible({ timeout: 60_000 })
  await expect(page.getByRole('button', { name: /Approval Timeout Output.*运行 \d+.*成功/ })).toBeVisible()
  ...
  await page.getByRole('dialog', { name: '确认取消执行？' }).getByRole('button', { name: '取消执行' }).click()
  await expect(page.locator('header').getByText('已取消', { exact: true })).toBeVisible({ timeout: 60_000 })

...
export function EntityFormDialog({ cancelLabel, fields, onClose, onSubmit, open, submitLabel, title }: Props) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setPending(true)
    setError('')
    try { await onSubmit(values); onClose() } catch (value) { setError(value instanceof Error ? value.message : String(value)) } finally { setPending(false) }
  }
  ...
  return <Dialog onOpenChange={(value) => { if (!value) onClose() }} open={open}>
    <DialogContent className={wide ? 'w-[min(920px,calc(100vw-48px))] p-6' : 'p-6'} title={title}>
      ...
      <form className={wide ? 'mt-5 grid grid-cols-2 gap-x-5 gap-y-4' : 'mt-5 space-y-4'} onSubmit={(event) => void submit(event)}>
        {fields.map((field) => <label className={field.type === 'textarea' && wide ? 'col-span-2 block text-xs' : 'block text-xs'} key={field.name}>
          ...
          {field.type === 'select' ? <Select className="w-full" onValueChange={(value) => update(field.name, value)} options={field.options ?? []} value={values[field.name]} />
            : field.type === 'textarea' ? <Textarea onChange={(event) => update(field.name, event.target.value)} placeholder={field.placeholder} required={field.required} value={values[field.name]} />
              : <Input max={field.max} min={field.type === 'number' ? (field.min ?? 0) : undefined} onChange={(event) => update(field.name, event.target.value)} placeholder={field.placeholder} required={field.required} step={field.step} type={field.type ?? 'text'} value={values[field.name]} />}
        </label>)}

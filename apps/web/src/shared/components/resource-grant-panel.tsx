...
export function ResourceGrantDialog({ onClose, open, resource }: { onClose: () => void; open: boolean; resource?: Resource }) {
  ...
  const operationOptions = operationsFor(resourceType).map((value) => ({ value, label: t(`m2.${value}`) }))
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setSubjectId(''); showToast(t('m2.grantAdded'))
  ...
  return <><Dialog onOpenChange={(value) => { if (!value) onClose() }} open={open}>
    <DialogContent className="w-[min(880px,calc(100vw-48px))] p-6" title={t('resourceGrants.manageTitle', { name: resource?.name ?? '' })}>
      ...
      {auth.hasPermission('resource:grant') && <form className="mt-6 grid grid-cols-[180px_minmax(220px,1fr)_160px_auto] items-end gap-3 rounded-lg border border-border bg-canvas/40 p-4" onSubmit={(event) => void submit(event)}>
        ...
        <Field label={t('resourceGrants.subject')}><Select onValueChange={setSubjectId} options={subjectOptions} placeholder={t('m2.select')} value={subjectId} /></Field>
        <Field label={t('m2.operation')}><Select onValueChange={setOperation} options={operationOptions} value={effectiveOperation} /></Field>
      ...
      <div className="mt-5 overflow-hidden rounded-lg border border-border">
        <div className="grid grid-cols-[180px_minmax(0,1fr)_120px_48px] bg-muted/45 px-4 py-2 text-[10px] font-medium uppercase text-muted-foreground"><span>{t('resourceGrants.subjectType')}</span><span>{t('resourceGrants.subject')}</span><span>{t('m2.operation')}</span><span /></div>
        {grants.isLoading && <p className="p-4 text-xs text-muted-foreground">{t('m2.loading')}</p>}
...
function operationsFor(type: string) { return type === 'rag' || type === 'memory' ? ['read', 'write', 'view', 'manage'] : ['use', 'view', 'manage'] }

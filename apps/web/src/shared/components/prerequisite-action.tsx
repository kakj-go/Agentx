...
export function PrerequisiteAction({ children, description, loading, onReady, requirements, ...buttonProps }: PrerequisiteActionProps) {
  ...
  return <>
    ...
    <Dialog onOpenChange={setOpen} open={open}>
      <DialogContent description={description ?? t('prerequisites.description')} title={t('prerequisites.title')}>
        <div className="p-5">
          <div className="flex items-start gap-3">
            ...
            <div><h2 className="text-sm font-semibold">{t('prerequisites.title')}</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">{description ?? t('prerequisites.description')}</p></div>
          </div>
          <div className="mt-5 divide-y divide-border rounded-lg border border-border">
            {requirements.map((requirement) => <div className="flex min-h-14 items-center gap-3 px-4" key={requirement.key}>
              ...
              {!requirement.met && !requirement.href && !requirement.onAction && <span className="text-[11px] text-muted-foreground">{t('prerequisites.contactAdmin')}</span>}
            </div>)}
          </div>

...
function WorkflowNode({ data }: NodeProps<Node<WorkflowNodeData>>) {
  ...
  return (
    <div className="min-w-52 overflow-hidden rounded-xl border border-border bg-surface shadow-lg">
      ...
      <div className="flex items-center justify-between px-3.5 py-2.5 text-[10px] text-muted-foreground"><span>{t('editor.status')}</span><span>{t('editor.configured')}</span></div>
...
export function WorkflowCanvas() {
  ...
  const initialNodes = useMemo<Array<Node<WorkflowNodeData>>>(() => [
    { id: 'trigger', position: { x: 80, y: 220 }, data: { label: t('editor.trigger'), detail: 'Chat Trigger', kind: 'trigger' }, type: 'workflow' },
    { id: 'agent', position: { x: 390, y: 160 }, data: { label: t('editor.agent'), detail: 'AI Agent · Tools', kind: 'agent' }, type: 'workflow' },
    { id: 'approval', position: { x: 720, y: 260 }, data: { label: t('editor.approval'), detail: 'Approval', kind: 'approval' }, type: 'workflow' },
  ], [t])
  ...
  return (
    <div className="flex h-full min-h-[calc(100vh-64px)] flex-col">
      <div className="flex h-15 shrink-0 items-center gap-3 border-b border-border bg-surface px-5">
        <div><h1 className="text-sm font-semibold">{t('editor.title')}</h1><p className="mt-0.5 text-[10px] text-muted-foreground">{t('editor.draft')}</p></div>
        <div className="flex-1" />
        <ComingSoonAction size="sm" variant="secondary"><Save className="size-3.5" />{t('editor.save')}</ComingSoonAction>
        <ComingSoonAction size="sm"><Play className="size-3.5" />{t('editor.run')}</ComingSoonAction>
      </div>
      <div className="relative min-h-0 flex-1 bg-canvas">
        ...
        <ComingSoonAction className="absolute left-5 top-5 z-10" size="sm"><Plus className="size-3.5" />{t('editor.addNode')}</ComingSoonAction>
      </div>
    </div>
  )
}

...
const manifest = (nodeType: string, variadic = false): NodeManifest => ({
  protocolVersion: '1.0', nodeType, version: 1, displayName: nodeType, description: '', category: 'actions', keywords: [], iconKey: 'box', executionStyle: 'action', capability: 'builtin', readiness: 'any',
  ...
  bindingSlots: nodeType === 'agent' ? [{ name: 'ai_model', resourceType: 'model', required: false, multiple: false }] : [], parameterSchema: {}, uiSchema: {}, providers: [], credentials: [],
...
describe('GraphIndex connection validation', () => {
  it('indexes nodes, ports, edges, bindings, and dynamic main handles', () => {
    const nodes = [action('source'), action('target', 'agent'), binding('binding:model')]
    const edges = [edge('flow', 'source', 'target'), edge('model', 'binding:model', 'target', 'resource', 'binding:ai_model')]
    ...
    expect(index.bindingSummaryByNodeId.get('target')?.[0]).toMatchObject({ role: 'ai_model', resourceType: 'model' })
    expect(occupiedHandlesByNodeId(index).get('source')).toBe('main')
    expect(occupiedHandlesByNodeId(index).get('target')).toBe('binding:ai_model')
  ...
  it('rejects self, duplicate, and incompatible connections and marks occupied inputs for replacement', () => {
    const nodes = [action('source'), action('other'), action('target'), action('agent', 'agent'), binding('binding:model')]
    const edges = [edge('existing', 'source', 'target'), edge('model', 'binding:model', 'agent', 'resource', 'binding:ai_model')]
    const manifests = new Map([['set@1', manifest('set')], ['agent@1', manifest('agent')]])
    const index = createGraphIndex(nodes, edges, manifests)
    ...
    expect(inspectConnection({ source: 'binding:model', sourceHandle: 'resource', target: 'agent', targetHandle: 'binding:ai_model' }, index)).toMatchObject({ status: 'invalid', reason: 'duplicate_connection' })
  })

...
describe('If panel', () => {
  ...
  it('initializes a fresh empty branch with one editable condition row', () => {
    function EmptyHarness() {
      ...
      return <><IfPanel data={current} fieldErrors={{}} manifest={manifest} onChange={(patch) => setCurrent((value) => ({ ...value, ...patch }))} providerOptions={{}} resources={{}} /><output data-testid="empty-state">{JSON.stringify(current.parameters)}</output></>
    }
    render(<EmptyHarness />)

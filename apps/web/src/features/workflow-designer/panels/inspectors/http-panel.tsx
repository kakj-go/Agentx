...
export function HttpPanel(panel: ActionPanelProps) {
  ...
  useEffect(() => {
    if (credentialType && credentialType !== "api_key" && panel.data.parameters.apiKeyPlacement) {
      ...
      delete parameters.apiKeyPlacement;
      panel.onChange({ parameters });
    }
...
function KeyValueRows({ name, panel }: { name: "query" | "headers"; panel: ActionPanelProps }) {
  ...
  const update = (next: typeof rows) => panel.onChange({ parameters: { ...panel.data.parameters, [name]: next } });
...
function ApiKeyPlacement({ panel }: { panel: ActionPanelProps }) {
  ...
  const update = (patch: { in?: string; name?: string }) => panel.onChange({ parameters: { ...panel.data.parameters, apiKeyPlacement: { in: value?.in ?? "header", name: value?.name ?? "x-api-key", ...patch } } });

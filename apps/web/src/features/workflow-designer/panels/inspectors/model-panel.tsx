    ...
    <div className="space-y-5" data-testid="model-panel">
      ...
      <PanelSection title={t("studio.panels.model.output")} testId="model-output-section">
        ...
        {structured && <div className="mt-4 space-y-2">
          {!panel.data.parameters.structuredSchema && <Button data-testid="model-schema-template" onClick={() => panel.onChange({ parameters: { ...panel.data.parameters, structuredSchema: { type: "object", properties: { answer: { type: "string" } }, required: ["answer"], additionalProperties: false } } })} size="sm" variant="secondary">{t("studio.panels.model.initializeSchema")}</Button>}

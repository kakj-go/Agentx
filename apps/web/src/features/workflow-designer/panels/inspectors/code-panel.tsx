...
function NetworkPolicyEditor({ panel }: { panel: ActionPanelProps }) {
  ...
  const update = (networkPolicy: NetworkPolicy) => panel.onChange({ parameters: { ...panel.data.parameters, networkPolicy } });
...
function OutputExampleEditor({ panel }: { panel: ActionPanelProps }) {
  ...
  return <div data-field-path="parameters.outputExample" data-testid="code-output-example">
    ...
    <div className="overflow-hidden rounded-md border border-border bg-canvas shadow-inner focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/15">
      <CodeEditor height="180px" language="json" onChange={(next) => {
      setSource(next);
      try {
        const parsed = JSON5.parse(next) as unknown;
        if (!isRecord(parsed)) throw new Error(t("studio.panels.code.outputObjectRequired"));
        setError(undefined);
        panel.onChange({ parameters: { ...panel.data.parameters, outputExample: parsed } });
      } catch (cause) {

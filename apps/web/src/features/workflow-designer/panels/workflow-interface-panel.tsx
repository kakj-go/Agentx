  ...
  return (
    <aside className="flex h-full w-[480px] shrink-0 flex-col overflow-hidden border-l border-border bg-surface" data-testid="workflow-interface-panel">
      <header className="flex h-16 items-center gap-3 border-b border-border px-4">
        <span className="grid size-9 place-items-center rounded-md bg-primary/10 text-primary"><LogIn className="size-5" /></span>
...
export function SectionTitle({ title, description, action }: { title: string; description?: string; action?: React.ReactNode }) { return <div className="mb-3 flex items-start justify-between gap-3"><div><h2 className="text-xs font-semibold">{title}</h2>{description && <p className="mt-1 text-[10px] text-muted-foreground">{description}</p>}</div>{action}</div>; }
...
export function inputKind(property: InputProperty) { if (property["x-agentx-artifact-array"]) return "artifact_array"; if (property["x-agentx-artifact"]) return "artifact"; return property.type ?? "string"; }
...
export function outputType(schema: unknown) { return String(asObject(schema).type ?? "string"); }

...
import { ReferenceControl, StructuredJsonControl } from "../forms/parameter-field";
...
const ERROR_FIELDS = [
  ...
  ["sourceNodeId", "string"], ["sourceNodeKey", "string"], ["nodeExecutionId", "string"],
  ["runIndex", "number"], ["iterationIndex", "number"], ["retryable", "boolean"],
] as const;
...
function InputFieldDialog({ open, onOpenChange, editing, properties, onSave }: { open: boolean; onOpenChange: (open: boolean) => void; editing: { name: string; property: InputProperty; required: boolean } | null; properties: Record<string, InputProperty>; onSave: (name: string, property: InputProperty, required: boolean) => void }) {
  ...
  return <Dialog onOpenChange={onOpenChange} open={open}><DialogContent description={t("studio.interface.fieldDialogDescription")} title={editing ? t("studio.editField") : t("studio.addField")}><div className="space-y-4 p-5"><div><h2 className="text-sm font-semibold">{editing ? t("studio.editField") : t("studio.addField")}</h2><p className="mt-1 text-[11px] text-muted-foreground">{t("studio.interface.fieldDialogDescription")}</p></div><div className="grid gap-3"><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.fieldType")}</span><Select onValueChange={setKind} options={INPUT_TYPES.map((value) => ({ value, label: t(`studio.schemaTypes.${value}`, value) }))} value={kind} /></label><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.fieldName")}</span><Input onChange={(e) => setName(e.target.value)} value={name} /></label><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.fieldTitle")}</span><Input onChange={(e) => setProperty((p) => ({ ...p, title: e.target.value }))} value={property.title ?? ""} /></label><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.description")}</span><Textarea className="min-h-14" onChange={(e) => setProperty((p) => ({ ...p, description: e.target.value }))} value={property.description ?? ""} /></label><label className="flex items-center gap-2 text-xs"><input checked={required} className="size-4 accent-primary" onChange={(e) => setRequired(e.target.checked)} type="checkbox" />{t("studio.interface.required")}</label>{(kind === "object" || kind === "array") && <SchemaShapeEditor onChange={(next) => setProperty(next as InputProperty)} schema={property} />}{(kind === "artifact" || kind === "artifact_array") && <ArtifactConstraints onChange={(patch) => setProperty((p) => ({ ...p, ...patch }))} property={property} t={translate} />}{showAdvanced && <details className="rounded-md border border-border/70 px-3 py-2" onToggle={(event) => setAdvancedOpen(event.currentTarget.open)} open={advancedOpen}><summary className="cursor-pointer text-xs font-medium text-muted-foreground">{t("studio.interface.constraintsAndDefault")}</summary><div className="mt-3"><TypeSpecificSettings onChange={(patch) => setProperty((p) => ({ ...p, ...patch }))} property={property} t={translate} /></div></details>}</div>{propertyError && <p className="text-xs text-danger" role="alert">{propertyError}</p>}<div className="flex justify-end gap-2 border-t border-border pt-4"><Button onClick={() => onOpenChange(false)} variant="ghost">{t("common.cancel")}</Button><Button disabled={!name.trim() || Boolean(propertyError)} onClick={() => onSave(name.trim(), property, required)}>{t("common.save")}</Button></div></div></DialogContent></Dialog>;
}
...
function ContextFieldDialog({ open, onOpenChange, editing, contexts, onSave }: { open: boolean; onOpenChange: (open: boolean) => void; editing: { name: string; context: ContextDefinition } | null; contexts: WorkflowStart["contexts"]; onSave: (name: string, context: ContextDefinition) => void }) {
  ...
  return <Dialog onOpenChange={onOpenChange} open={open}><DialogContent description={t("studio.interface.contextDialogDescription") } title={editing ? t("studio.editContext") : t("studio.addContext")}><div className="space-y-4 p-5"><div><h2 className="text-sm font-semibold">{editing ? t("studio.editContext") : t("studio.addContext")}</h2><p className="mt-1 text-[11px] text-muted-foreground">{t("studio.interface.contextDialogDescription")}</p></div><div className="grid gap-3"><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.fieldType")}</span><Select onValueChange={setType} options={INPUT_TYPES.slice(0, 5).map((value) => ({ value, label: t(`studio.schemaTypes.${value}`, value) }))} value={type} /></label><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.contextName")}</span><Input onChange={(e) => setName(e.target.value)} value={name} /></label><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.fieldTitle")}</span><Input onChange={(e) => setContext((c) => ({ ...c, title: e.target.value }))} value={context.title ?? ""} /></label><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.description")}</span><Textarea className="min-h-16" onChange={(e) => setContext((c) => ({ ...c, description: e.target.value }))} value={context.description ?? ""} /></label><DefaultControl onChange={(value) => setContext((c) => ({ ...c, default: value }))} property={schemaWithDefault} t={translate} /><label className="flex items-center gap-2 text-xs"><input checked={context.sensitive} className="size-4 accent-primary" onChange={(e) => setContext((c) => ({ ...c, sensitive: e.target.checked }))} type="checkbox" />{t("studio.interface.sensitive")}</label>{(type === "object" || type === "array") && <SchemaShapeEditor onChange={(next) => setContext((c) => ({ ...c, schema: next }))} schema={schema} />}<details className="rounded-md border border-border/70 px-3 py-2" onToggle={(event) => setAdvancedOpen(event.currentTarget.open)} open={advancedOpen}><summary className="cursor-pointer text-xs font-medium text-muted-foreground">{t("studio.interface.advanced")}</summary><div className="mt-3 grid gap-3"><div className="grid grid-cols-2 gap-2 text-xs"><label className="flex items-center gap-2"><input checked={context.mutable} className="size-4 accent-primary" onChange={(e) => setContext((c) => ({ ...c, mutable: e.target.checked }))} type="checkbox" />{t("studio.interface.mutable")}</label><label className="flex items-center gap-2"><input checked={context.clientWritable} className="size-4 accent-primary" onChange={(e) => setContext((c) => ({ ...c, clientWritable: e.target.checked }))} type="checkbox" />{t("studio.interface.clientWritable")}</label></div><div className="grid grid-cols-2 gap-2"><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.scope")}</span><Select onValueChange={(value) => setContext((c) => ({ ...c, scope: value as ContextDefinition["scope"] }))} options={[{ value: "execution_tree", label: t("studio.contextScopes.execution_tree") }, { value: "session", label: t("studio.contextScopes.session") }]} value={context.scope} /></label><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.mergePolicy")}</span><Select onValueChange={(value) => setContext((c) => ({ ...c, mergePolicy: value as ContextDefinition["mergePolicy"] }))} options={contextMergePolicies(type).map((value) => ({ value, label: t(`studio.mergePolicies.${value}`, value) }))} value={context.mergePolicy} /></label></div><LabeledNumberInput label={t("studio.interface.maxSize")} min={1} onChange={(maxSize) => setContext((c) => ({ ...c, maxSize }))} value={context.maxSize} />{type === "array" ? <ArrayConstraintFields onChange={(patch) => setContext((c) => ({ ...c, schema: { ...schema, ...patch } }))} property={schema} t={translate} /> : type === "object" ? <label className="flex items-center gap-2 text-xs"><input checked={schema.additionalProperties !== false} className="size-4 accent-primary" onChange={(event) => setContext((c) => ({ ...c, schema: { ...schema, additionalProperties: event.target.checked } }))} type="checkbox" />{t("studio.interface.additionalProperties")}</label> : <ConstraintFields onChange={(patch) => setContext((c) => ({ ...c, schema: { ...schema, ...patch } }))} property={schema} t={translate} />}</div></details></div><div className="flex justify-end gap-2 border-t border-border pt-4"><Button onClick={() => onOpenChange(false)} variant="ghost">{t("common.cancel")}</Button><Button disabled={!name.trim()} onClick={() => onSave(name.trim(), context)}>{t("common.save")}</Button></div></div></DialogContent></Dialog>;
}
...
function EndForm({ end, onChange, referenceCatalog }: { end: WorkflowEnd; onChange: (value: ValueUpdater<WorkflowEnd>) => void; referenceCatalog?: ReferenceCatalog }) {
  ...
  const errorCatalog = referenceCatalog ? { ...referenceCatalog, item: [{ id: "item.json", label: t("studio.references.currentError"), path: "item.json", expression: "${{ item.json }}", type: "object", children: ERROR_FIELDS.map(([name, type]) => ({ id: `item.json.${name}`, label: t(`studio.errorFields.${name}`, name), path: `item.json.${name}`, expression: `\${{ item.json.${name} }}`, type, children: [] })) }] } : undefined;
  return <><OutputForm referenceCatalog={referenceCatalog} title={t("studio.interface.successOutputs")} outputs={end.outputs} onChange={(update) => onChange((current) => ({ ...current, outputs: resolveUpdate(current.outputs, update) }))} /><section><SectionTitle description={t("studio.interface.errorOutputsDescription")} title={t("studio.interface.errorOutputs")} /><div className="grid gap-2"><Select aria-label={t("studio.interface.errorStrategy")} onValueChange={(strategy) => onChange((current) => ({ ...current, error: { ...normalizedError(current.error), strategy: strategy as WorkflowEnd["error"]["strategy"] } }))} options={[{ value: "fail_fast", label: t("studio.errorStrategies.fail_fast") }, { value: "collect", label: t("studio.errorStrategies.collect") }]} value={error.strategy} />{error.strategy === "collect" && <Input aria-label={t("studio.interface.collectWindow")} min={100} max={60000} onChange={(event) => { const collectWindowMs = Math.min(60000, Math.max(100, Number(event.target.value) || 5000)); onChange((current) => ({ ...current, error: { ...normalizedError(current.error), collectWindowMs } })); }} type="number" value={String(error.collectWindowMs)} />}</div><div className="mt-5"><OutputForm currentError referenceCatalog={errorCatalog} title={t("studio.interface.errorFields")} outputs={error.outputs} onChange={(update) => onChange((current) => { const currentError = normalizedError(current.error); return { ...current, error: { ...currentError, outputs: resolveUpdate(currentError.outputs, update) } }; })} /></div></section></>;
}
...
function OutputForm({ title, outputs, onChange, referenceCatalog, currentError = false }: { title: string; outputs: Record<string, WorkflowOutput>; onChange: (value: ValueUpdater<Record<string, WorkflowOutput>>) => void; referenceCatalog?: ReferenceCatalog; currentError?: boolean }) {
  ...
  const add = () => {
    ...
    const output = { expression: "", schema: { type: "string" }, required: false, sensitive: false } satisfies WorkflowOutput;
    onChange((current) => ({ ...current, [name]: output }));
  ...
  const remove = (name: string) => onChange((current) => { const next = { ...current }; delete next[name]; return next; });
  const updateDraft = (name: string, output: WorkflowOutput) => onChange((current) => ({ ...current, [name]: output }));
  const renameDraft = (name: string, nextName: string) => {
    const clean = nextName.trim();
    if (!clean || clean === name || outputs[clean]) return name;
    onChange((current) => renamedProperty(current, name, clean));
    setEditing((current) => current ? { ...current, name: clean } : current);
    return clean;
  };
  const cancel = () => {
    if (editing) onChange((current) => {
      if (editing.isNew) { const next = { ...current }; delete next[editing.name]; return next; }
      const restored = renamedProperty(current, editing.name, editing.originalName);
      return { ...restored, [editing.originalName]: editing.original };
    });
    setEditing(null);
    setDialogOpen(false);
  };
  const save = () => { setEditing(null); setDialogOpen(false); };
  return <section><SectionTitle action={<Button onClick={add} size="sm" variant="secondary"><Plus className="size-3.5" />{t("studio.addField")}</Button>} title={title} /><div className="space-y-2">{Object.entries(outputs).map(([name, output]) => <OutputSummary key={name} name={name} output={output} onDelete={() => remove(name)} onEdit={() => edit(name, output)} />)}</div><OutputFieldDialog currentError={currentError} editing={editing ? { ...editing, output: outputs[editing.name] ?? editing.original } : null} onCancel={cancel} onChange={(name, output) => { const nextName = name === editing?.name ? name : renameDraft(editing?.name ?? name, name); updateDraft(nextName, output); setEditing((current) => current ? { ...current, name: nextName } : current); }} onOpenChange={(open) => open ? setDialogOpen(true) : cancel()} onSave={save} open={dialogOpen} referenceCatalog={referenceCatalog} /> </section>;
}
...
function OutputSummary({ name, output, onEdit, onDelete }: { name: string; output: WorkflowOutput; onEdit: () => void; onDelete: () => void }) {
  ...
  return <div className="flex items-center gap-3 rounded-md border border-border px-3 py-2.5"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="truncate text-xs font-medium">{String(schema.title ?? name)}</span><span className="rounded bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground">{t(`studio.schemaTypes.${outputType(output.schema)}`, outputType(output.schema))}</span>{output.required && <span className="text-[10px] text-danger">{t("studio.interface.required")}</span>}{output.sensitive && <span className="text-[10px] text-warning">{t("studio.interface.sensitive")}</span>}</div><div className="truncate font-mono text-[10px] text-muted-foreground">{name}</div>{output.expression && <div className="truncate text-[10px] text-muted-foreground">{output.expression}</div>}</div><Button aria-label={t("studio.editField")} onClick={onEdit} size="icon" variant="ghost"><Pencil className="size-3.5" /></Button><Button aria-label={t("studio.removeField")} onClick={onDelete} size="icon" variant="ghost"><Trash2 className="size-3.5" /></Button></div>;
}

function OutputFieldDialog({ open, onOpenChange, editing, referenceCatalog, currentError, onChange, onSave, onCancel }: { open: boolean; onOpenChange: (open: boolean) => void; editing: { name: string; output: WorkflowOutput } | null; referenceCatalog?: ReferenceCatalog; currentError: boolean; onChange: (name: string, output: WorkflowOutput) => void; onSave: () => void; onCancel: () => void }) {
  const { t } = useTranslation();
  const [name, setName] = useState("");
  const [output, setOutput] = useState<WorkflowOutput>({ expression: "", schema: { type: "string" }, required: false, sensitive: false });
  const wasOpen = useRef(false);
  useEffect(() => {
    if (open && !wasOpen.current && editing) { setName(editing.name); setOutput(editing.output); }
    wasOpen.current = open;
  }, [open, editing]);
  const update = (patch: Partial<WorkflowOutput>) => { const next = { ...output, ...patch }; setOutput(next); onChange(name, next); };
  const updateName = (nextName: string) => { setName(nextName); onChange(nextName, output); };
  const updateType = (type: string) => update({ schema: { ...schemaForType(type), title: asObject(output.schema).title, description: asObject(output.schema).description } });
  const schema = asObject(output.schema);
  const allowed = currentError ? ["inputs", "outputs", "contexts", "item"] as const : ["inputs", "outputs", "contexts"] as const;
  return <Dialog onOpenChange={onOpenChange} open={open}><DialogContent description={t("studio.interface.outputDialogDescription")} title={t("studio.interface.outputField")}><div className="space-y-4 p-5"><div><h2 className="text-sm font-semibold">{t("studio.interface.outputField")}</h2><p className="mt-1 text-[11px] text-muted-foreground">{t("studio.interface.outputDialogDescription")}</p></div><div className="grid gap-3"><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.fieldType")}</span><Select aria-label={t("studio.interface.fieldType")} onValueChange={updateType} options={INPUT_TYPES.slice(0, 5).map((value) => ({ value, label: t(`studio.schemaTypes.${value}`, value) }))} value={outputType(output.schema)} /></label><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.outputName")}</span><Input aria-label={t("studio.interface.outputName")} onChange={(event) => updateName(event.target.value)} value={name} /></label><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.fieldTitle")}</span><Input onChange={(event) => update({ schema: { ...schema, title: event.target.value } })} value={String(schema.title ?? "")} /></label><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.content")}</span><ReferenceExpressionInput allowed={[...allowed]} ariaLabel={t("studio.expression")} catalog={referenceCatalog} onChange={(expression) => update({ expression })} placeholder={currentError ? "${{ item.json.message }}" : "${{ outputs.node.main.current.json }}"} value={output.expression} /></label>{(outputType(output.schema) === "object" || outputType(output.schema) === "array") && <SchemaShapeEditor onChange={(next) => update({ schema: next })} schema={output.schema as JsonSchemaProperty} />}<div className="grid grid-cols-2 gap-3 text-xs"><label className="flex items-center gap-2"><input checked={output.required} className="size-4 accent-primary" onChange={(event) => update({ required: event.target.checked })} type="checkbox" />{t("studio.interface.required")}</label><label className="flex items-center gap-2"><input checked={output.sensitive} className="size-4 accent-primary" onChange={(event) => update({ sensitive: event.target.checked })} type="checkbox" />{t("studio.interface.sensitive")}</label></div><label className="text-xs"><span className="mb-1 block text-muted-foreground">{t("studio.interface.description")}</span><Textarea className="min-h-14" onChange={(event) => update({ schema: { ...schema, description: event.target.value } })} value={String(schema.description ?? "")} /></label></div><div className="flex justify-end gap-2 border-t border-border pt-4"><Button onClick={onCancel} variant="ghost">{t("common.cancel")}</Button><Button disabled={!name.trim()} onClick={onSave}>{t("common.save")}</Button></div></div></DialogContent></Dialog>;
}

function ReferenceExpressionInput({ allowed, catalog, value, placeholder, onChange, ariaLabel }: { allowed: Array<"inputs" | "outputs" | "contexts" | "item">; catalog?: ReferenceCatalog; value: string; placeholder: string; onChange: (value: string) => void; ariaLabel?: string }) {
  const { t } = useTranslation();
  ...
  return <ReferenceControl allowed={allowed} catalog={catalog} enabled={Boolean(catalog)} onInsert={insert}><Input aria-label={ariaLabel ?? t("studio.expression")} className="font-mono text-xs" onChange={(event) => onChange(event.target.value)} placeholder={placeholder} ref={input} value={value} /></ReferenceControl>;
}
...
function artifactReferenceSchema(): JsonSchemaProperty { return { type: "object", required: ["artifactId"], properties: { artifactId: { type: "string", format: "uuid" }, type: { type: "string" }, fileName: { type: "string" }, contentType: { type: "string" }, sizeBytes: { type: "number", minimum: 0 } }, additionalProperties: false }; }
...
function renamedProperty<T extends Record<string, unknown>>(value: T, oldName: string, nextName: string): T { const clean = nextName.trim(); if (!clean || clean === oldName || value[clean]) return value; const next = { ...value, [clean]: value[oldName] } as T; delete next[oldName]; return next; }

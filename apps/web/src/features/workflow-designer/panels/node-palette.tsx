import { ArrowLeft, ChevronDown, FileText, Layers3, LogOut, Plus, Search, X } from 'lucide-react'
...
type PaletteProps = { manifests: NodeManifest[]; sourceConnection?: { manifest: NodeManifest; handleId: string }; bindingSlot?: BindingSlot; open?: boolean; onOpenChange?: (open: boolean) => void; onAddAction: (manifest: NodeManifest, targetHandle?: string) => void; onAddBinding: (resourceType: ResourceType, role: string) => void; onAddExit?: () => void; onAddAnnotation?: () => void; onAddGroup?: () => void }
type PaletteGroupKey = NodeCategory | 'attachments' | 'exits'

export function NodePalette({ manifests, sourceConnection, bindingSlot, open, onOpenChange, onAddAction, onAddBinding, onAddExit, onAddAnnotation, onAddGroup }: PaletteProps) {
  ...
  const groupKeys = useMemo<PaletteGroupKey[]>(() => [...visibleGroups.keys(), ...(!sourceConnection && attachments.length ? ['attachments' as const] : []), ...(!sourceConnection && onAddExit ? ['exits' as const] : [])], [attachments.length, onAddExit, sourceConnection, visibleGroups])
  ...
  return <aside className="pointer-events-none absolute inset-y-0 left-0 z-30" data-testid="node-creator-shell">
    ...
    {expanded && <div className="pointer-events-auto absolute inset-y-0 left-0 z-30 flex w-80 flex-col border-r border-border bg-surface shadow-xl" data-testid="node-creator" onKeyDown={handleCreatorKeyDown}>

...
import type { Checkpoint, RuntimeDetails } from '../../../shared/api/types'
...
export function RuntimePanel({ workflowId, executionId, events, onExecutionChange }: { workflowId: string; executionId?: string; events: ExecutionEvent[]; onExecutionChange: (executionId: string) => void }) {
  ...
  const executionOptions = (executions.data?.items ?? []).filter((execution) => execution.workflowId === workflowId).map((execution) => ({ value: execution.id, label: `${execution.status} · ${new Date(execution.startedAt).toLocaleString()}` }))
  ...
  if (collapsed) return <section className="relative flex h-10 shrink-0 items-center gap-3 border-t border-border bg-surface px-4" data-testid="runtime-rail"><button aria-label={t('studio.runtime.expand', 'Expand execution rail')} className="flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground" onClick={() => setCollapsed(false)} type="button"><ChevronUp className="size-4" />{t('studio.runtime.title', 'Execution')}</button>{executionSelect}</section>
  return <section className="relative shrink-0 border-t border-border bg-surface px-4" data-testid="runtime-rail" style={{ height }}><button aria-label={t('studio.runtime.resize')} className="absolute -top-2 left-1/2 z-10 grid h-4 w-10 -translate-x-1/2 cursor-row-resize place-items-center rounded-sm border border-border bg-surface text-muted-foreground" onPointerDown={beginResize} type="button"><GripHorizontal className="size-3" /></button><Tabs className="h-full" defaultValue="events"><TabsList className="h-10 gap-5"><TabsTrigger className="text-xs" value="events">{t('studio.runtime.events')}</TabsTrigger><TabsTrigger className="text-xs" value="trace">{t('studio.runtime.trace')}</TabsTrigger><TabsTrigger className="text-xs" value="checkpoints">{t('studio.runtime.checkpoints')}</TabsTrigger><span className="flex-1" />{executionSelect}<Button aria-label={t('studio.runtime.collapse', 'Collapse execution rail')} onClick={() => setCollapsed(true)} size="icon" variant="ghost"><ChevronDown className="size-3.5" /></Button></TabsList>
      <TabsContent className="h-[calc(100%-40px)] overflow-auto py-3" value="events">{events.map((event) => <div className="grid grid-cols-[60px_160px_100px_1fr] gap-3 border-b border-border py-1.5 text-[10px]" key={event.sequence}><span>#{event.sequence}</span><span>{event.eventType}</span><span>{event.status}</span><span className="truncate font-mono text-muted-foreground">{JSON.stringify(event.summary)}</span></div>)}</TabsContent>

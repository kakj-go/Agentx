...
export function RuntimePanel({ workflowId, executionId, events, onExecutionChange, onNodeSelect }: { workflowId: string; executionId?: string; events: ExecutionEvent[]; onExecutionChange: (executionId: string) => void; onNodeSelect?: (nodeId: string) => void }) {
  ...
  const executions = useQuery({ queryKey: ['studio-executions', workflowId], queryFn: () => apiRequest<PageResponse<Execution>>(`/executions?workflowIds=${encodeURIComponent(workflowId)}&limit=100`), enabled: Boolean(workflowId), refetchInterval: (query) => { if (!executionId || !query.state.data?.items) return false; const execution = query.state.data.items.find((e: Execution) => e.id === executionId); return execution && !['succeeded', 'failed', 'timed_out', 'cancelled'].includes(execution.status) ? 3000 : false } })

...
export function buildReferenceCatalog(
  ...
  catalog.outputs = document.nodes.flatMap((node) => {
    ...
    const manifest = resolveNodeManifest(manifests, nodeData.nodeType, nodeData.typeVersion, nodeData.parameters);
    if (!manifest) return [];
...
function effectiveNodeOutputSchema(
  ...
  const manifest = resolveNodeManifest(manifests, data.nodeType, data.typeVersion, data.parameters);
  if (!manifest) return {};

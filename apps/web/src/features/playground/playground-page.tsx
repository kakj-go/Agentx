...
export function PlaygroundPage() {
  ...
  const createSession = useMutation({
    mutationFn: () => gatewayRequest<GatewaySession>(`/applications/${selected?.slug}/sessions`, { method: 'POST', body: jsonBody({ title: t('m3.playgroundSession'), externalUserId: null }) }),
  ...
  return <PageContainer className="flex min-h-full flex-col">
    <PageHeader action={<div className="flex items-center gap-2"><Select className="w-64" onValueChange={(value) => { setApplicationId(value); setSession(null); setInvocationId(undefined) }} options={(applications.data?.items ?? []).map((item) => ({ value: item.id, label: item.name }))} placeholder={t('m3.selectApplication')} value={applicationId} /><Button disabled={!selected || createSession.isPending} onClick={() => createSession.mutate()} variant="secondary"><MessageSquarePlus className="size-4" />{t('pages.playground.newSession')}</Button></div>} description={t('pages.playground.description')} title={t('pages.playground.title')} />
    <Card className="mt-6 grid min-h-[620px] flex-1 grid-cols-[280px_minmax(0,1fr)] overflow-hidden">
      <aside className="border-r border-border bg-muted/25"><div className="flex h-14 items-center border-b border-border px-4 text-sm font-semibold">{t('pages.playground.sessions')}</div><div className="p-2">{session ? <div className="rounded-md bg-primary/10 p-3"><strong className="block text-xs text-primary">{session.title ?? selected?.name}</strong><span className="mt-1 block truncate text-[10px] text-muted-foreground">{session.id}</span></div> : <p className="p-4 text-xs text-muted-foreground">{t('m3.noSession')}</p>}</div></aside>
      <section className="flex min-w-0 flex-col">
        <div className="flex h-14 items-center gap-3 border-b border-border px-5"><span className="grid size-8 place-items-center rounded-md bg-primary/10 text-primary"><Bot className="size-4" /></span><div><strong className="block text-xs">{selected?.name ?? t('m3.selectApplication')}</strong><span className="text-[10px] text-muted-foreground">{session?.versionPolicy ?? '/gateway/v1'}</span></div>{invocation.data && <><Badge className="ml-auto" tone={invocation.data.status === 'failed' ? 'danger' : running ? 'warning' : 'success'}>{invocation.data.status}</Badge>{invocation.data.executionId && <Button asChild size="icon" variant="ghost"><Link aria-label={t('m3.trace')} to={`/executions/${invocation.data.executionId}`}><ExternalLink className="size-4" /></Link></Button>}{running && <Button aria-label={t('studio.stop')} disabled={cancel.isPending} onClick={() => cancel.mutate()} size="icon" variant="ghost"><Square className="size-4" /></Button>}</>}</div>
        {invocation.data?.errorMessage && <div className="border-b border-danger/25 bg-danger/10 px-5 py-2 text-xs text-danger"><strong>{invocation.data.errorCode ?? 'APPLICATION_INVOCATION_FAILED'}</strong><span className="ml-2">{invocation.data.errorMessage}</span></div>}
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">{!session ? <EmptyConversation icon={<UserRound className="size-6" />} title={t('m3.selectAndCreate')} /> : messages.isLoading ? <p className="text-xs text-muted-foreground">{t('m2.loading')}</p> : (messages.data?.length ?? 0) === 0 ? <EmptyConversation icon={<Bot className="size-6" />} title={t('m7.noMessages')} /> : <div className="space-y-4">{messages.data?.map((item) => <article className={`flex gap-3 ${item.role === 'user' ? 'justify-end' : 'justify-start'}`} key={item.id}><div className={`max-w-[75%] rounded-md px-4 py-3 text-xs leading-5 ${item.role === 'user' ? 'bg-primary text-primary-foreground' : 'border border-border bg-muted/40 text-foreground'}`}>{item.parts.map((part, index) => <div className="whitespace-pre-wrap break-words" key={`${part.partType}-${index}`}>{formatPart(part.content)}</div>)}<span className={`mt-2 block text-[10px] ${item.role === 'user' ? 'text-primary-foreground/70' : 'text-muted-foreground'}`}>{new Date(item.createdAt).toLocaleTimeString()}</span></div></article>)}</div>}</div>
        <div className="border-t border-border p-4"><div className="flex gap-2"><Input disabled={!session || running} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && message.trim() && !send.isPending && !running) send.mutate() }} placeholder={t('pages.playground.composer')} value={message} /><Button aria-label={t('pages.playground.send')} disabled={!session || !message.trim() || send.isPending || running} onClick={() => send.mutate()} size="icon"><Send className="size-4" /></Button></div></div>
      </section>
    </Card>
  </PageContainer>
}

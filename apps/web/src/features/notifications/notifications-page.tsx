...
export function NotificationsPage() {
  ...
  return <PageContainer><PageHeader action={inbox.data?.unreadCount ? <Button onClick={() => read.mutate(undefined)} variant="secondary"><CheckCheck className="size-4" />{t('m3.readAll')}</Button> : undefined} description={t('m3.notificationsDescription')} title={t('common.notifications')} />
    <Card className="mt-6 overflow-hidden">{inbox.data?.items.length ? <div className="divide-y divide-border">{inbox.data.items.map((item) => <button className="flex w-full items-start gap-4 px-5 py-4 text-left transition-colors hover:bg-muted/40" key={item.id} onClick={() => void open(item.id, item.targetPath)}><span className={`mt-1.5 size-2 shrink-0 rounded-full ${item.read ? 'bg-border' : item.tone === 'danger' ? 'bg-danger' : item.tone === 'warning' ? 'bg-warning' : item.tone === 'success' ? 'bg-success' : 'bg-primary'}`} /><span className="min-w-0 flex-1"><strong className="text-xs">{t(item.titleKey, item.arguments as Record<string, string>)}</strong><span className="mt-1 block text-[11px] leading-5 text-muted-foreground">{t(item.bodyKey, item.arguments as Record<string, string>)}</span></span><time className="text-[10px] text-muted-foreground">{new Date(item.createdAt).toLocaleString()}</time></button>)}</div> : <EmptyState icon={Bell} title={t('header.noNotifications')} description={t('m3.noNotificationsDescription')} />}</Card>
  </PageContainer>
}

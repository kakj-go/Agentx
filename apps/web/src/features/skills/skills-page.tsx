...
export function SkillsPage() {
  ...
  const create = useMutation({ mutationFn: (values: Record<string, string>) => apiRequest<Skill>('/skills', { method: 'POST', body: jsonBody({ name: values.name, description: values.description, ownerDepartmentId: values.department }) }), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ['skills'] }); showToast(t('m2.created')) } })
  const columns = useMemo<Array<ColumnDef<Skill>>>(() => [
    { accessorKey: 'name', header: t('table.skill'), cell: ({ row }) => <EntityCell detail={t('m2.workspaceRevision', { revision: row.original.draftRevision })} icon={Sparkles} name={row.original.name} /> },
  ...
  const fields: EntityFormField[] = [{ name: 'name', label: t('common.name'), required: true }, { name: 'description', label: t('m2.description'), type: 'textarea', required: true }, { name: 'department', label: t('m2.department'), type: 'select', required: true, options: (departments.data ?? []).map((item) => ({ value: item.id, label: item.name })) }]
  return <>
    <ListPage action={auth.hasPermission('skill:manage') ? <Button onClick={() => setOpen(true)}><Plus className="size-4" />{t('m2.createSkill')}</Button> : undefined} columns={columns} data={skills.data?.items ?? []} description={t('pages.skills.description')} getSearchText={(row) => `${row.name} ${row.description ?? ''}`} getStatus={(row) => row.status === 'active' ? 'active' : row.status === 'draft' ? 'draft' : 'inactive'} searchPlaceholder={t('pages.skills.search')} statusOptions={[{ value: 'active', label: t('common.active') }, { value: 'draft', label: t('common.draft') }, { value: 'inactive', label: t('common.inactive') }]} title={t('pages.skills.title')} />

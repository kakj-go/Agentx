...
export function CredentialDetailPage() {
  ...
  const rotateFields: EntityFormField[] = [{ name: 'secret', label: t('m2.secret'), type: 'password', required: true, placeholder: t('m2.passwordMask') }]
  const actions = auth.hasPermission('credential:manage') && value ? <div className="flex gap-2">
    ...
    <Button onClick={() => setRotateOpen(true)} variant="secondary"><RefreshCw className="size-4" />{t('m2.rotate')}</Button>
    <Button disabled={toggle.isPending} onClick={() => toggle.mutate()} variant="secondary"><Power className="size-4" />{value.status === 'active' ? t('m2.disable') : t('m2.enable')}</Button>
  </div> : undefined
  return <>
    <ResourceDetailLayout actions={actions} description={t('m2.credentialDescription')} details={value ? [
      { label: t('m2.credentialType'), value: value.credentialType },
      { label: t('m2.storageMode'), value: value.storageMode },
      { label: t('m2.secret'), value: value.maskedHint },
      ...
      <Card className="flex items-center gap-3 p-4 text-xs text-muted-foreground"><KeyRound className="size-4 text-primary" />{t('m2.passwordMask')}</Card>
    </ResourceDetailLayout>
    {rotateOpen && <EntityFormDialog cancelLabel={t('common.cancel')} fields={rotateFields} onClose={() => setRotateOpen(false)} onSubmit={rotate} open submitLabel={t('m2.rotate')} title={t('m2.rotate')} />}
    {editOpen && value && <EntityFormDialog cancelLabel={t('common.cancel')} fields={[{ name: 'name', label: t('common.name'), defaultValue: value.name, required: true }]} onClose={() => setEditOpen(false)} onSubmit={rename} open submitLabel={t('common.save')} title={t('m2.renameCredential')} />}
  </>
}

...
export function ApprovalsPage() {
  ...
  return <Tabs onValueChange={(value) => setSearchParams(value === 'resource-grants' ? { tab: value } : {})} value={activeTab}>
    ...
    <TabsContent value="resource-grants"><ListPage columns={requestColumns} data={resourceRequests.data?.items ?? []} description={t('approvals.resourceGrantDescription')} getSearchText={(row) => `${row.primaryResourceName ?? ''} ${row.primaryResourceType} ${row.workflowName} ${row.requestedByName}`} getStatus={(row) => resourceRequestStatus(row.status)} searchPlaceholder={t('approvals.resourceGrantSearch')} statusOptions={[{ value: 'pending', label: t('common.pending') }, { value: 'active', label: t('approvals.resourceGrantStatuses.approved') }, { value: 'inactive', label: t('common.inactive') }]} tableHeader={tabs} title={t('approvals.title')} /></TabsContent>
  </Tabs>
}

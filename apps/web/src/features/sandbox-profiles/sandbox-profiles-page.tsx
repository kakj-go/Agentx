...
export function SandboxProfilesPage() {
  ...
  return <>
    <ListPage action={auth.hasPermission('sandbox:manage') ? <Button onClick={() => setOpen(true)}><Plus className="size-4" />{t('sandbox.create')}</Button> : undefined} columns={columns} data={profiles.data?.items ?? []} description={t('pages.sandboxProfiles.description')} getSearchText={(row) => `${row.name} ${row.current.runner} ${row.current.imageDigest}`} getStatus={(row) => row.status} searchPlaceholder={t('pages.sandboxProfiles.search')} statusOptions={[{ value: 'active', label: t('common.active') }, { value: 'disabled', label: t('common.inactive') }]} title={t('pages.sandboxProfiles.title')} />

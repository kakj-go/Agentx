...
export function WorkflowDetailPage() {
  ...
  const rollback = async (deployment: WorkflowDeployment) => { await apiRequest(`/workflows/${workflowId}/deployments/${deployment.environmentId}/rollback`, { method: 'POST', headers: { 'Idempotency-Key': crypto.randomUUID() }, body: jsonBody({ targetWorkflowVersionId: deployment.workflowVersionId }) }); await invalidate(); showToast(t('workflows.saved')) }
  ...
  const publishFields: EntityFormField[] = [
    { name: 'environment', label: t('workflows.selectEnvironment'), type: 'select', required: true, options: (environments.data ?? []).filter((item) => item.status === 'active').map((item) => ({ value: item.id, label: item.name })) },
  ...
  return <PageContainer>
    ...
    <div className="mt-5 flex items-center gap-3 text-xs"><StatusBadge status={value.status === 'active' ? 'active' : 'inactive'} /><span>{t('workflows.revision', { revision: value.draftRevision })}</span><span>{value.latestVersion ? t('workflows.versionNumber', { version: value.latestVersion }) : t('workflows.draftLabel')}</span><span className="text-muted-foreground">{value.ownerName}</span></div>
    <div className="mt-6 grid grid-cols-3 gap-5">
      <Card className="col-span-2 p-5"><h2 className="text-sm font-semibold">{t('workflows.resources')}</h2><div className={`mt-4 rounded-lg border p-4 text-xs ${validation.data?.valid ? 'border-success/20 bg-success/10 text-success' : 'border-warning/20 bg-warning/10 text-warning'}`}><div className="flex items-center gap-2"><ShieldCheck className="size-4" />{validation.data?.valid ? t('workflows.validationPassed') : t('workflows.validationFailed', { count: validation.data?.missingGrants.length ?? 0 })}</div>{validation.data?.missingGrants.map((issue) => <p className="mt-2 font-mono text-[11px]" key={`${issue.nodeId}-${issue.resourceId}-${issue.operation}`}>{issue.nodeId}: {localizedValue(t, 'resourceGrants.resourceTypes', issue.resourceType)}/{issue.resourceId} · {localizedValue(t, 'resourceGrants.validationReasons', issue.reason)}</p>)}</div></Card>
      <Card className="p-5"><h2 className="text-sm font-semibold">{t('workflows.overview')}</h2><dl className="mt-4 space-y-3 text-xs"><Info label={t('workflows.visibility')} value={localizedValue(t, 'workflows.visibilities', value.visibility)} /><Info label={t('workflows.workflowId')} value={value.id} /><Info label={t('workflows.serviceIdentity')} value={value.serviceIdentityId} /></dl>{auth.hasPermission('workflow:archive') && value.status === 'active' && <Button className="mt-5 w-full" disabled={archive.isPending} onClick={() => { if (window.confirm(t('workflows.confirmArchive'))) archive.mutate() }} variant="secondary"><Archive className="size-4" />{t('workflows.archive')}</Button>}</Card>
      ...
      <SectionCard action={auth.hasPermission('workflow:publish') ? <Button disabled={createVersion.isPending || value.status !== 'active'} onClick={() => createVersion.mutate()} size="sm" variant="secondary"><Plus className="size-3.5" />{t('workflows.createVersion')}</Button> : undefined} icon={GitBranch} title={t('workflows.versions')}>{versions.data?.map((version) => <div className="flex items-center border-t border-border py-3 first:border-t-0" key={version.id}><div className="min-w-0"><p className="text-xs font-medium">v{version.versionNumber}</p><p className="mt-1 truncate text-[11px] text-muted-foreground">{t('workflows.sourceRevision', { revision: version.sourceRevision })} · {version.contentHash.slice(0, 20)}…</p></div><div className="flex-1" />{auth.hasPermission('execution:run') && <Button disabled={runVersion.isPending} onClick={() => runVersion.mutate(version.id)} size="sm" variant="ghost"><Play className="size-3.5" />{t('executions.runVersion')}</Button>}</div>)}{versions.data?.length === 0 && <Empty />}</SectionCard>
      <SectionCard icon={Play} title={t('workflows.deployments')}>{deployments.data?.map((deployment) => <div className="flex items-center border-t border-border py-3 first:border-t-0" key={deployment.id}><div><p className="text-xs font-medium">{deployment.environmentName} · v{deployment.versionNumber}</p><p className="mt-1 text-[11px] text-muted-foreground">{localizedValue(t, 'common', deployment.status)} · {localizedValue(t, 'workflows.deploymentSources', deployment.source)}</p></div><div className="flex-1" />{auth.hasPermission('workflow:publish') && deployment.status !== 'active' && <Button onClick={() => void rollback(deployment)} size="sm" variant="ghost">{t('workflows.rollback')}</Button>}</div>)}{deployments.data?.length === 0 && <Empty />}</SectionCard>
    </div>
    {publishOpen && <EntityFormDialog cancelLabel={t('common.cancel')} fields={publishFields} onClose={() => setPublishOpen(false)} onSubmit={publish} open submitLabel={t('workflows.publish')} title={t('workflows.publish')} />}

...
function Field({ children, className = '', label }: { children: React.ReactNode; className?: string; label: string }) { return <label className={`space-y-1.5 text-xs font-medium ${className}`}><span>{label}</span>{children}</label> }

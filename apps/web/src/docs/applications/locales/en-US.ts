export const applicationIntegrationDocs = {
  ...
  webhook: {
    ...
    description: 'Trigger the active application deployment with a dedicated URL and HMAC signature.',
    intro: 'You can inspect the Webhook guide and deployed schemas before creating a Webhook. Creation generates the dedicated URL and one-time signing secret.',
    noSpecificEndpoint: 'No specific Webhook is selected. Examples use WEBHOOK_PUBLIC_ID as a placeholder. Open the guide from a Webhook row to fill in its dedicated URL.',
    stepsTitle: 'Integration steps',
    steps: ['Inspect the active deployment input fields under Schemas.', 'Create a Webhook and immediately store its one-time secret.', 'Serialize the JSON body first, then calculate the Unix timestamp and HMAC signature.', 'Send the exact same body bytes and reuse Idempotency-Key when retrying the same event.'],
    signingTitle: 'Signed invocation',
    signatureTitle: 'Signature algorithm',
    signatureFormula: "Base64UrlNoPadding(HMAC-SHA256(secret, timestamp + '.' + rawBody))",
    fields: [
      ['Path', 'public_id', 'string', true, 'Public route ID generated when the Webhook is created.'], ['Header', 'Idempotency-Key', 'string (1–192)', true, 'Unique event value; retries of the same event must reuse it.'], ['Header', 'X-Agentx-Timestamp', 'Unix integer', true, 'Unix timestamp in seconds, within five minutes of server time.'],
      ['Header', 'X-Agentx-Signature', 'Base64URL string', true, 'Unpadded HMAC-SHA256 over the raw body bytes.'], ['Header', 'Content-Type', 'application/json', true, 'Only JSON is currently accepted.'], ['Body', '$', 'object', true, 'Workflow input that must match the input schema.'],
    ],
    securityItems: [
      ['Dedicated signing secret', 'The Webhook secret belongs only to this Webhook. It is not an API key.'], ['Sign the raw body', 'Serialize first, then sign exactly the bytes sent; never reformat JSON after signing.'], ['Replay protection', 'Timestamps older than five minutes are rejected and Idempotency-Key prevents duplicate execution.'], ['Store securely', 'The secret is shown only at creation and must not enter logs, source code, or clients.'],
    ],
  },
  commonErrors: [
    ['400', 'The JSON body, input schema, or required headers are invalid.'], ['401', 'The API key, Webhook signature, or timestamp is invalid.'], ['404', 'The application route, Invocation, or Webhook does not exist or is inactive.'], ['409', 'The same Idempotency-Key was used with different request content.'], ['422', 'Request semantics or application input validation failed.'], ['429', 'The application or tenant rate limit was exceeded.'], ['503', 'Runtime or a dependency is temporarily unavailable; retry with the same Idempotency-Key.'],
  ],
} as const

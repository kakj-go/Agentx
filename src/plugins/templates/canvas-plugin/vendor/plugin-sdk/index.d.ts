...
export type Item = {
    ...
    binary?: Record<string, ArtifactRef>;
};
...
export type ExecuteContext<P extends Json = Json> = {
    ...
    signal: AbortSignal;
    trace: {
        span<T>(name: string, body: (span: TraceSpan) => Promise<T> | T): Promise<T>;
    };
    items: {
        fromJson(values: Json[]): Item[];
    };
...
export declare function completed(outputs: NodeOutputs): Completed;
export declare function failed(code: string, message: string, retryable?: boolean, details?: Json): Failed;
export declare function defineExecute<P extends Json>(execute: Execute<P>): Execute<P>;
export type ResolvedDefinition = {
    ...
    outputSchema?: Json;
    outputPortSchemas?: Record<string, Json>;
    issues?: Array<{
        path: string;
        code: string;
        message: string;
...
export type ResolveDefinition<P extends Json = Json> = (configuration: P, upstreamContracts: Record<string, Json>, context: {
    nodeType: string;
}) => Promise<ResolvedDefinition> | ResolvedDefinition;
...
export type Provider = (input: {
    ...
    cursor?: string;
    parameters?: Json;
}, context: ProviderContext) => Promise<ProviderOption[] | ProviderPage> | ProviderOption[] | ProviderPage;

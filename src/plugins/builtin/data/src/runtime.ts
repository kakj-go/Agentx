import type { Execute, Item, Json, ResolveDefinition } from '@agentx/plugin-sdk'

type Parameters = Record<string, Json>

export const resolveDefinition: ResolveDefinition<Parameters> = (configuration, upstreamContracts, context) => {
  if (context.nodeType === 'set') {
    ...
    const schema = configuration.keepOnlySet === true
      ? { type: 'object', properties: {}, required: [], additionalProperties: false } as Record<string, Json>
...
export const execute: Execute<Parameters> = (context) => {
  if (context.execution.nodeType === 'set') return executeSet(context.inputs, context.parameters, context.perItemParameters)
  if (context.execution.nodeType === 'list') return executeList(context.parameters, context.perItemParameters)
  return { status: 'failed', code: 'BUILTIN_NODE_UNSUPPORTED', message: `Unsupported data node ${context.execution.nodeType}` }
}

function executeSet(inputs: Record<string, Item[]>, common: Parameters, perItem: Parameters[]) {
  ...
  return { status: 'completed' as const, outputs: { main: output } }
}

function executeList(common: Parameters, perItem: Parameters[]) {
  if (!Array.isArray(common.input)) return { status: 'failed' as const, code: 'LIST_INPUT_NOT_ARRAY', message: 'List input must resolve to an array' }
  const kept = common.input.map((json, index) => ({ index, item: { json } as Item })).filter(({ index }) => passes(perItem[index] ?? common))
  const fields = Array.isArray(common.sort) ? common.sort : []
  if (fields.length) kept.sort((left, right) => compareFields(perItem[left.index] ?? common, perItem[right.index] ?? common, fields))
  const take = typeof common.takeN === 'number' && common.takeN >= 0 ? common.takeN : kept.length
  return { status: 'completed' as const, outputs: { main: [{ json: { items: kept.slice(0, take).map(({ item }) => item.json) } }] } }
}

function passes(parameters: Parameters) {
...
function compareFields(left: Parameters, right: Parameters, fields: Json[]) {
  ...
  for (let index = 0; index < fields.length; index += 1) {
    ...
    if (ordering !== 0) return specification.direction === 'desc' ? -ordering : ordering
  }
  return 0
}

function compareJson(left: Json | undefined, right: Json | undefined, nullsFirst: boolean) {
  const leftMissing = left === null || left === undefined
  const rightMissing = right === null || right === undefined
  if (leftMissing && rightMissing) return 0
  if (leftMissing) return nullsFirst ? -1 : 1
  if (rightMissing) return nullsFirst ? 1 : -1
...
function schemaObject(value: Json | undefined): Record<string, Json> | undefined {
  return value && typeof value === 'object' && !Array.isArray(value) ? value : undefined
}

function cloneSchema(value: Json): Record<string, Json> {
  const cloned = JSON.parse(JSON.stringify(value)) as Json
  return schemaObject(cloned) ?? { type: 'object' }
}

function bindingFields(value: Json | undefined): Record<string, Json> {
  const binding = object(value)
  return binding.kind === 'object' ? object(binding.fields) : {}
}

function bindingSchema(value: Json | undefined, upstream: Record<string, Json>): Json {
  const binding = object(value)
  switch (binding.kind) {
    ...
    case 'reference': {
      ...
      if (selector.namespace === 'inputs') return schemaAt(upstream.$inputs, path)
...
function listConditionIssues(configuration: Parameters, upstream: Record<string, Json>) {
  ...
  return rows.flatMap((entry, index) => {
    ...
    if (schemasCompatible(left, right, operator)) return []
...
function schemasCompatible(left: Json, right: Json, operator: string) {
  const leftTypes = schemaTypes(left)
  const rightTypes = schemaTypes(right)
  if (!leftTypes.length || !rightTypes.length) return true
  ...
  if (['contains', 'not_contains', 'starts_with', 'ends_with', 'matches'].includes(operator)) return leftTypes.includes('string') || leftTypes.includes('array')
...
function schemaTypes(value: Json) {
...
function schemaAt(value: Json | undefined, path: Json[]): Json {
...
function literalSchema(value: Json | undefined): Json {
  ...
  if (typeof value === 'object') return { type: 'object', properties: Object.fromEntries(Object.entries(value).map(([name, child]) => [name, literalSchema(child)])), required: Object.keys(value) }

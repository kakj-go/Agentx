...
export function DataTable<T>({ columns, data, getSearchText, searchPlaceholder, getStatus, statusOptions = [], getCategory, categoryOptions = [], categoryLabel, tableHeader }: DataTableProps<T>) {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) => {
      ...
      const matchesCategory = category === 'all' || getCategory?.(row) === category
      return matchesSearch && matchesStatus && matchesCategory
    })
  }, [category, data, getCategory, getSearchText, getStatus, search, status])
  ...
  useEffect(() => table.setPageIndex(0), [category, search, status, table])
    ...
    <Card className="mt-6 overflow-hidden">
      ...
      <div className="flex min-h-16 items-center gap-3 border-b border-border px-5">
        ...
        {categoryOptions.length > 0 && (
          <Select
            aria-label={categoryLabel ?? t('common.status')}

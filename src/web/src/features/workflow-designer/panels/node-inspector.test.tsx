...
const agentManifest: NodeManifest = {
  ...
  version: 2,
  capability: 'agent',
  plugin: { packageId: 'agentx/core', packageVersion: '1.0.0', bundleDigest: 'sha256:test', runtimeEntry: 'native', runtimeSource: '' },
  bindingSlots: [
    ...
    { name: 'mcp_tools', resourceType: 'mcp_tool', placement: 'inspector', required: false, multiple: true },
    { name: 'skills', resourceType: 'skill', placement: 'inspector', required: false, multiple: true },
    { name: 'knowledge', resourceType: 'rag', placement: 'inspector', required: false, multiple: true },
    { name: 'long_term_memory', resourceType: 'memory', placement: 'inspector', required: false, multiple: false },
  ],
  parameterSchema: { type: 'object', properties: { systemPrompt: { type: 'string' }, userQuestion: { type: 'string' }, maxIterations: { type: 'integer', default: 12 } } },
  uiSchema: { canvas: { role: 'agent' }, fields: { systemPrompt: { control: 'prompt' }, userQuestion: { control: 'template' }, maxIterations: { control: 'number' } } },
}
...
const modelManifest: NodeManifest = {
  ...
  parameterSchema: { type: 'object', properties: { prompt: { type: 'string' }, userQuestion: { type: 'string' } } },
  uiSchema: { canvas: { role: 'default' }, fields: { prompt: { control: 'prompt' }, userQuestion: { control: 'template' } } },
...
describe('NodeInspector details view', () => {
  it('provides the fixed Parameters, Input, Output and Trace views', () => {
    ...
    render(<QueryClientProvider client={client}><ToastProvider><NodeInspector data={{ editorKind: 'action', nodeType: 'set', typeVersion: 1, label: 'Set', key: 'set', parameters: {}, contextWrites: [], resourceReferences: [], settings: {}, disabled: false }} manifest={manifest} nodeId="node-1" onChange={vi.fn()} onDelete={vi.fn()} onRun={vi.fn()} resources={{}} /></ToastProvider></QueryClientProvider>)

    expect(screen.getByTestId('node-details-view')).toHaveStyle({ width: '440px' })
  ...
  it('presents Agent prompt and one user question before an inline advanced section', () => {
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    render(<QueryClientProvider client={client}><ToastProvider><NodeInspector data={{ editorKind: 'action', nodeType: 'agent', typeVersion: 1, label: 'Agent', key: 'agent', parameters: {}, contextWrites: [], resourceReferences: [], settings: {}, disabled: false }} manifest={agentManifest} nodeId="agent-1" onChange={vi.fn()} onDelete={vi.fn()} resources={{}} /></ToastProvider></QueryClientProvider>)

    expect(screen.getByTestId('parameter-systemPrompt')).toBeInTheDocument()
    expect(screen.getByTestId('parameter-userQuestion').querySelector('[contenteditable="true"]')).toBeInTheDocument()
  ...
  it('configures Agent model, optional Workspace Sandbox and explicit Session Policy in the Inspector', () => {
    ...
    const model = { bindingRole: 'model', resourceType: 'model' as const, resourceId: 'model-1', resourceVersionId: 'model-version-1', operation: 'use' as const }
    const sandbox = { bindingRole: 'workspace_sandbox', resourceType: 'sandbox_profile' as const, resourceId: 'sandbox-1', resourceVersionId: 'sandbox-version-1', operation: 'use' as const }
    const data = { editorKind: 'action' as const, nodeType: 'agent', typeVersion: 2, label: 'Agent', key: 'agent', parameters: { sessionPolicy: { mode: 'invocation' } }, contextWrites: [], resourceReferences: [model, sandbox], settings: {}, disabled: false }
    ...
    expect(screen.getByTestId('agent-inspector-mcp_tools')).toBeInTheDocument()
    expect(screen.getByTestId('agent-inspector-skills')).toBeInTheDocument()
    expect(screen.getByTestId('agent-inspector-knowledge')).toBeInTheDocument()
    expect(screen.getByTestId('agent-inspector-long_term_memory')).toBeInTheDocument()
  ...
  it('moves node disablement into the more menu and shows disabled status in the header', () => {
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    const onChange = vi.fn()
    render(<QueryClientProvider client={client}><ToastProvider><NodeInspector data={{ editorKind: 'action', nodeType: 'set', typeVersion: 1, label: 'Set', key: 'set', parameters: {}, contextWrites: [], resourceReferences: [], settings: {}, disabled: true }} manifest={manifest} nodeId="node-1" onChange={onChange} onDelete={vi.fn()} resources={{}} /></ToastProvider></QueryClientProvider>)
  ...
  it('shows Model prompt and one user question without the raw parameters field', () => {
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    render(<QueryClientProvider client={client}><ToastProvider><NodeInspector data={{ editorKind: 'action', nodeType: 'model', typeVersion: 1, label: 'Model', key: 'model', parameters: {}, contextWrites: [], resourceReferences: [], settings: {}, disabled: false }} manifest={modelManifest} nodeId="model-1" onChange={vi.fn()} onDelete={vi.fn()} resources={{}} /></ToastProvider></QueryClientProvider>)
    ...
    expect(question.querySelector('[contenteditable="true"]')).toBeInTheDocument()
    expect(screen.queryByTestId('parameter-parameters')).not.toBeInTheDocument()
  })

  it('exposes the current input item to node parameter bindings', () => {
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    const referenceCatalog = { inputs: [], outputs: [], contexts: [], item: [{ id: 'item.value', label: 'value', path: 'item.value', type: 'string', schema: { type: 'string' }, selector: { namespace: 'item' as const, run: { kind: 'current' as const }, item: { kind: 'current' as const }, path: ['value'] }, children: [] }] }
    const itemAwareModel: NodeManifest = {
      ...modelManifest,
      parameterSchema: { type: 'object', properties: { ...modelManifest.parameterSchema.properties, userQuestion: { type: 'string', "x-agentx-binding": { acceptedKinds: ['literal', 'reference', 'template'], allowedNamespaces: ['inputs', 'outputs', 'contexts', 'item'], acceptedCardinality: ['single'], missingPolicies: ['error'], recursive: false } } } },
      uiSchema: { ...modelManifest.uiSchema, fields: { ...modelManifest.uiSchema.fields, userQuestion: { control: 'template' } } },
    }
    render(<QueryClientProvider client={client}><ToastProvider><NodeInspector data={{ editorKind: 'action', nodeType: 'model', typeVersion: 1, label: 'Model', key: 'model', parameters: { userQuestion: '' }, contextWrites: [], resourceReferences: [], settings: {}, disabled: false }} manifest={itemAwareModel} nodeId="model-1" onChange={vi.fn()} onDelete={vi.fn()} referenceCatalog={referenceCatalog} resources={{}} /></ToastProvider></QueryClientProvider>)

    fireEvent.click(within(screen.getByTestId('parameter-userQuestion')).getByRole('textbox', { name: 'Value' }))

    expect(screen.getByRole('button', { name: /Current data|当前数据/ })).toBeInTheDocument()
  })

  it('configures context writes from the advanced section', () => {
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    const onChange = vi.fn()
    ...
    render(<QueryClientProvider client={client}><ToastProvider><NodeInspector data={{ editorKind: 'action', nodeType: 'model', typeVersion: 1, label: 'Model', key: 'model', parameters: {}, contextWrites: [], resourceReferences: [], settings: {}, disabled: false }} manifest={modelManifest} nodeId="model-1" onChange={onChange} onDelete={vi.fn()} referenceCatalog={referenceCatalog} resources={{}} /></ToastProvider></QueryClientProvider>)

    fireEvent.click(screen.getByText(/Advanced configuration|高级配置/))
  ...
  it('does not pin the previous node output while the selected node is loading', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock)
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    const agentData = { editorKind: 'action' as const, nodeType: 'agent', typeVersion: 1, label: 'Agent', key: 'agent', parameters: {}, contextWrites: [], resourceReferences: [], settings: {}, disabled: false }
  ...
  it('refreshes the selected node until its output is available to pin', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock)
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    const codeData = { editorKind: 'action' as const, nodeType: 'set', typeVersion: 1, label: 'Code', key: 'code', parameters: {}, contextWrites: [], resourceReferences: [], settings: {}, disabled: false }

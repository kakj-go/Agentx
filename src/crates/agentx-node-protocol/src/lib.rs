...
pub const NODE_PROTOCOL_VERSION: &str = "3.0";
...
impl NodeCapability {
    ...
    pub const fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::PluginNodejs => "plugin_nodejs",
            Self::Agent => "agent",
            Self::Model => "model",
...
pub struct PluginNodeBinding {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub runtime_artifact: Option<PluginRuntimeArtifact>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub ui_entry: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub ui_source: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub ui_styles: Option<String>,
    #[serde(default, skip_serializing_if = "BTreeMap::is_empty")]
    pub ui_assets: BTreeMap<String, String>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub trace_renderers: Vec<PluginTraceRenderer>,
}

#[derive(Clone, Debug, Deserialize, JsonSchema, PartialEq, Serialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct PluginRuntimeArtifact {
    pub object_id: Uuid,
...
#[must_use]
pub fn plugin_runtime_object_id(bundle_digest: &str) -> Uuid {
    use sha2::{Digest, Sha256};

    let digest = Sha256::digest(format!("agentx-plugin-runtime-v1:{bundle_digest}").as_bytes());
...
#[derive(Clone, Debug, Deserialize, JsonSchema, PartialEq, Serialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct PluginTraceRenderer {
...
impl NodeManifestVersion {
    ...
    pub fn validate_plugin(&self) -> Result<(), String> {
        match (&self.capability, &self.plugin) {
            (NodeCapability::PluginNodejs, Some(plugin)) => {
                use sha2::Digest as _;

                let runtime_hash = format!(
                    "sha256:{:x}",
                    sha2::Sha256::digest(plugin.runtime_source.as_bytes())
                );
                if plugin.package_id.trim().is_empty()
                    || plugin.package_version.trim().is_empty()
                    || plugin.runtime_entry.trim().is_empty()
                    || plugin.runtime_source.trim().is_empty()
                    || !plugin.bundle_digest.starts_with("sha256:")
                    || plugin.runtime_artifact.as_ref().is_none_or(|artifact| {
                        artifact.object_id != plugin_runtime_object_id(&plugin.bundle_digest)
                            || artifact.content_hash != runtime_hash
                            || artifact.size_bytes != plugin.runtime_source.len() as u64
                            || artifact.content_hash.len() != 71
                            || !artifact.content_hash.starts_with("sha256:")
                            || artifact.size_bytes == 0
                            || artifact.media_type != "text/javascript"
                    })
                {
                    return Err("plugin_nodejs requires a complete immutable plugin binding".into());
                }
                if plugin.trace_renderers.iter().any(|renderer| {
                    renderer.content_type.trim().is_empty()
                        || renderer.content_version == 0
                        || renderer.export_name.trim().is_empty()
                }) {
                    return Err("plugin Trace renderer declaration is invalid".into());
                }
                Ok(())
            }
            (NodeCapability::PluginNodejs, None) => {
                Err("plugin_nodejs requires a plugin binding".into())
            }
            (_, Some(plugin))
                if plugin.package_id == "agentx/core"
                    && plugin.package_version == "1.0.0"
                    && plugin.bundle_digest.starts_with("sha256:")
                    && plugin.runtime_entry == "native"
                    && plugin.runtime_source.is_empty()
                    && plugin.runtime_artifact.is_none()
                    && plugin.ui_entry.is_none()
                    && plugin.ui_source.is_none()
                    && plugin.trace_renderers.is_empty() =>
            {
                Ok(())
            }
            (_, Some(_)) => Err("only plugin_nodejs may carry a plugin binding".into()),
            (_, None) => Ok(()),
        }
    }
...
mod tests {
    ...
    fn binding_slot_placement_is_required_and_inspector_supports_multi_select() {
        ...
        .expect("placement is part of Manifest 3.0");
    ...
    #[test]
    fn plugin_runtime_artifact_identity_is_deterministic() {
        let digest = format!("sha256:{}", "a".repeat(64));
        assert_eq!(
            plugin_runtime_object_id(&digest),
            plugin_runtime_object_id(&digest)
        );
        assert_ne!(
            plugin_runtime_object_id(&digest),
            plugin_runtime_object_id(&format!("sha256:{}", "b".repeat(64)))
        );
    }
}
